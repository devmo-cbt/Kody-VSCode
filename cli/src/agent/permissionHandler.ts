 ...
 * and Kody's internal permission system. It maps KodyAsk types to appropriate
 * ACP permission options and translates user responses back to Kody's format.
 *
 * @module acp/permissionHandler
 */

import type * as acp from "@agentclientprotocol/sdk"
import type { KodyAsk } from "@shared/ExtensionMessage"
import type { KodyAskResponse } from "@shared/WebviewMessage"
import { Logger } from "@/shared/services/Logger.js"
import type { AcpSessionState, KodyPermissionOption } from "./types.js"

/**
 * Standard permission options for operations that support "always allow".
 * Used for commands, tools, and MCP server operations.
 */
const STANDARD_PERMISSION_OPTIONS: KodyPermissionOption[] = [
 ...
 */
const RESTRICTED_PERMISSION_OPTIONS: KodyPermissionOption[] = [
	{ kind: "allow_once", optionId: "allow_once", name: "Allow Once" },
	{ kind: "reject_once", optionId: "reject_once", name: "Reject" },
]

/**
 * Mapping of KodyAsk types to their permission option sets.
 */
const ASK_TYPE_PERMISSION_MAP: Partial<Record<KodyAsk, KodyPermissionOption[]>> = {
...
/**
 * KodyAsk types that require permission handling.
 * Other ask types (like followup, plan_mode_respond) don't need permission UI.
 */
const PERMISSION_REQUIRING_ASK_TYPES: Set<KodyAsk> = new Set([
	"command",
	"tool",
...
/**
 * Check if a KodyAsk type requires permission handling.
 *
 * @param askType - The KodyAsk type to check
 * @returns True if the ask type requires permission UI
 */
export function requiresPermission(askType: KodyAsk): boolean {
	return PERMISSION_REQUIRING_ASK_TYPES.has(askType)
}

/**
 * Get the appropriate permission options for a KodyAsk type.
 *
 * @param askType - The KodyAsk type
 * @returns Array of permission options, or undefined if the ask type doesn't require permission
 */
export function getPermissionOptionsForAskType(askType: KodyAsk): acp.PermissionOption[] | undefined {
...
/**
 * Handle an ACP permission response and translate it to Kody's format.
 *
 * @param response - The ACP permission response from the client
 * @param askType - The original KodyAsk type that triggered the permission request
 * @returns The translated result for Kody's handleWebviewAskResponse
 */
export function handlePermissionResponse(response: acp.RequestPermissionResponse, askType: KodyAsk): PermissionHandlerResult {
	// Check if cancelled
	...
	// Translate the option to Kody's response format
	switch (optionId) {
		case "allow_once":
			return {
 ...
 * @param askType - The Kody ask type
...
export class AutoApprovalTracker {
	 ...
	 *
	 * @param askType - The Kody ask type that was auto-approved
	 * @param identifier - The identifier for the operation (command, tool name, etc.)
	 */
	recordAlwaysAllow(askType: KodyAsk, identifier: string): void {
		switch (askType) {
			case "command":
	 ...
	 */
	isAutoApproved(askType: KodyAsk, identifier: string): boolean {
		switch (askType) {
			case "command":
				const commandPrefix = identifier.split(" ")[0]
 ...
 * 4. Returns the translated result for Kody
 *
 ...
 */
export function getAutoApprovalIdentifier(toolCall: acp.ToolCall, askType: KodyAsk): string {
