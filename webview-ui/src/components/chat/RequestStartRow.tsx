...
const getActivityText = (tool: KodySayTool): string | null => {
...
const collectToolsInRange = (
	...
	startIdx: number,
	endIdx: number,
	stopCondition?: (msg: KodyMessage) => boolean,
...
const findCurrentApiReq = (messages: KodyMessage[]): { index: number; hasCost: boolean } | null => {
...
const findPrevCompletedApiReq = (messages: KodyMessage[], beforeIdx: number): number => {
...
export const RequestStartRow: React.FC<RequestStartRowProps> = ({
	...
	const hasCompletionResult = kodyMessages.some(
	...
	const willBeAbsorbed = useMemo(() => {
		return isApiReqAbsorbable(message.ts, kodyMessages)
	}, [message.ts, kodyMessages])
	...
	const currentActivities = useMemo(() => {
		const currentApiReq = findCurrentApiReq(kodyMessages)
		if (!currentApiReq) {
			return []
		}
		...
		if (!currentApiReq.hasCost) {
			...
			return collectToolsInRange(kodyMessages, currentApiReq.index + 1, kodyMessages.length)
		}
	...
	const hasCompletedTools = useMemo(() => {
		...
		return kodyMessages.some((msg, idx) => {
