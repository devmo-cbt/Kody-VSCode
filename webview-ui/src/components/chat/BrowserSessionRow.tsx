...
import { BrowserAction, BrowserActionResult, KodyMessage, KodySayBrowserAction } from "@shared/ExtensionMessage"
	...
	const pages = useMemo(() => {
		const result: {
			currentState: {
				...
				consoleLogs?: string
				messages: KodyMessage[] // messages up to and including the result
			}
			nextAction?: {
				messages: KodyMessage[] // messages leading to next result
			}
		}[] = []

		let currentStateMessages: KodyMessage[] = []
		let nextActionMessages: KodyMessage[] = []
	...
	const [browserSessionRow, { height }] = useSize(
		...
		<BrowserSessionRowContainer style={{ marginBottom: -10 }}>
			<div style={browserSessionRowContainerInnerStyle}>
				...
				<span style={approveTextStyle}>{isAutoApproved ? "Kody is using the browser:" : "Use browser:"}</span>
			</div>
			<div
				style={{
					borderRadius: 3,
