...
import {
	...
	KodyAskUseMcpServer,
	KodyMessage,
	KodyPlanModeResponse,
	KodySayGenerateExplanation,
	KodySayTool,
} from "@shared/ExtensionMessage"
import { BooleanRequest, StringRequest } from "@shared/proto/kody/common"
...
interface ChatRowProps {
	...
	lastModifiedMessage?: KodyMessage
	isLast: boolean
	...
	({
		...
		const [cost, apiReqCancelReason, apiReqStreamingFailedMessage] = useMemo(() => {
			if (message.text != null && message.say === "api_req_started") {
				const info: KodyApiReqInfo = JSON.parse(message.text)
			...
			switch (type) {
				...
				case "mistake_limit_reached":
					return [
						...
						<span className="text-error font-bold">Kody is having trouble...</span>,
					]
				case "command":
					return [
				...
				case "use_mcp_server":
					const mcpServerUse = JSON.parse(message.text || "{}") as KodyAskUseMcpServer
					return [
						...
						<span className="ph-no-capture font-bold text-foreground break-words">
							{mcpServerUse.type === "use_mcp_tool" ? "Use tool" : "Access resource"} on{" "}
		...
		const tool = useMemo(() => {
			if (message.ask === "tool" || message.say === "tool") {
				return JSON.parse(message.text || "{}") as KodySayTool
			}
			return null
		...
		if (tool) {
			...
			switch (tool.tool) {
				case "editedExistingFile":
					...
					const editToolTitle = isApplyingPatch ? "Kody is creating patches to edit this file:" : "Edit this file:"
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "This file is outside of your workspace")}
								<span className="font-bold">Create file:</span>
							</div>
							{backgroundEditEnabled && tool.path && tool.content ? (
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "This file is outside of your workspace")}
								<span className="font-bold">Read this file:</span>
							</div>
							...
							<div className={HEADER_CLASSNAMES}>
								...
								<span style={{ fontWeight: "bold" }}>
									{message.type === "ask"
										...
										: "Kody viewed the top level files in this directory:"}
								</span>
							</div>
							<CodeAccordian
								code={tool.content!}
							...
							<div className={HEADER_CLASSNAMES}>
								...
								<span style={{ fontWeight: "bold" }}>
									{message.type === "ask"
										...
										: "Kody recursively viewed all files in this directory:"}
								</span>
							</div>
							<CodeAccordian
								code={tool.content!}
							...
							<div className={HEADER_CLASSNAMES}>
								...
								<span style={{ fontWeight: "bold" }}>
									{message.type === "ask"
										...
										: "Kody viewed source code definition names used in this directory:"}
								</span>
							</div>
							<CodeAccordian
								code={tool.content!}
						...
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">
									Search directory for <code className="break-all">{tool.regex}</code>:
								</span>
							</div>
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">Kody is condensing the conversation:</span>
							</div>
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">
									{message.type === "ask" ? "Fetch: this URL:" : "Kody fetched content from this URL:"}
								</span>
							</div>
							<div
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">
									{message.type === "ask" ? "Web search:" : "Kody searched the web for:"}
								</span>
							</div>
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">Kody loaded the skill:</span>
							</div>
		...
		if (message.ask === "use_mcp_server" || message.say === "use_mcp_server") {
			const useMcpServer = JSON.parse(message.text || "{}") as KodyAskUseMcpServer
			...
			case "say":
				switch (message.say) {
					...
					case "kodyignore_error":
						return <ErrorRow errorType="kodyignore_error" message={message} />
					...
					case "generate_explanation": {
						let explanationInfo: KodySayGenerateExplanation = {
							title: "code changes",
							fromRef: "",
							toRef: "",
							status: "generating",
						...
						return (
							<div className="flex flex-col bg-warning/20 p-2 rounded-xs border border-error">
								...
								<div className="text-foreground opacity-80">
									Kody may have trouble viewing the command's output. Please update VSCode (
									<code>CMD/CTRL + Shift + P</code> → "Update") and make sure you're using a supported shell:
									zsh, bash, fish, or PowerShell (<code>CMD/CTRL + Shift + P</code> → "Terminal: Select Default
									Profile").
									<a
										className="px-1"
										href="https://github.com/kody/kody/wiki/Troubleshooting-%E2%80%90-Shell-Integration-Unavailable">
			...
			case "ask":
				switch (message.ask) {
					...
					case "followup":
						...
						try {
							const parsedMessage = JSON.parse(message.text || "{}") as KodyAskQuestion
					...
					case "new_task":
						return (
							<div>
								<div className={HEADER_CLASSNAMES}>
									<FilePlus2Icon className="size-2" />
									<span className="text-foreground font-bold">Start new task:</span>
								</div>
					...
					case "report_bug":
						return (
							<div>
								<div className={HEADER_CLASSNAMES}>
									<FilePlus2Icon className="size-2" />
									<span className="text-foreground font-bold">Create a Github issue:</span>
								</div>
					...
					case "plan_mode_respond": {
						...
						let selected: string | undefined
						try {
							const parsedMessage = JSON.parse(message.text || "{}") as KodyPlanModeResponse
