...
import type { KodyMessage, KodySayBrowserAction, KodySayTool } from "@shared/ExtensionMessage"
 ...
 */
export function isLowStakesTool(message: KodyMessage): boolean {
	if (message.say !== "tool" && message.ask !== "tool") {
		return false
	}
	try {
		const tool = JSON.parse(message.text || "{}") as KodySayTool
 ...
 */
export function isToolGroup(item: KodyMessage | KodyMessage[]): item is KodyMessage[] & { _isToolGroup: true } {
 ...
 */
export function processMessages(messages: KodyMessage[]): KodyMessage[] {
 ...
 */
export function filterVisibleMessages(messages: KodyMessage[]): KodyMessage[] {
	return messages.filter((message, index, arr) => {
		switch (message.ask) {
			case "completion_result":
				// don't show a chat row for a completion_result ask without text. This specific type of message only occurs if kody wants to execute a command as part of its completion result, in which case we interject the completion_result tool with the execute_command tool.
				if (message.text === "") {
					return false
				}
				break
		...
		switch (message.say) {
			...
			case "text":
				// Sometimes kody returns an empty text message, we don't want to render these. (We also use a say text for user messages, so in case they just sent images we still render that)
 ...
 */
export function isBrowserSessionMessage(message: KodyMessage): boolean {
	if (message.type === "ask") {
 ...
 */
export function groupMessages(visibleMessages: KodyMessage[]): (KodyMessage | KodyMessage[])[] {
	const result: (KodyMessage | KodyMessage[])[] = []
	let currentGroup: KodyMessage[] = []
		...
		if (message.ask === "browser_action_launch" || message.say === "browser_action_launch") {
			...
			if (isBrowserSessionMessage(message)) {
				...
				if (message.say === "browser_action") {
					const browserAction = JSON.parse(message.text || "{}") as KodySayBrowserAction
 ...
 */
export function getTaskMessage(messages: KodyMessage[]): KodyMessage | undefined {
 ...
 */
function isDisplayedCheckpoint(checkpointIndex: number, allMessages: KodyMessage[]): boolean {
	...
	for (let i = checkpointIndex - 1; i >= 0; i--) {
		...
		if (msg.say === "tool" || msg.ask === "tool") {
			try {
				const tool = JSON.parse(msg.text || "{}") as KodySayTool
 ...
 */
export function findNextSegmentCost(checkpointTs: number, allMessages: KodyMessage[]): number | undefined {
	const checkpointIndex = allMessages.findIndex((m) => m.ts === checkpointTs && m.say === "checkpoint_created")
	if (checkpointIndex === -1) {
		return undefined
	}
 ...
 */
export function isTextMessagePendingToolCall(textTs: number, allMessages: KodyMessage[]): boolean {
 ...
 */
export function isToolGroupInFlight(toolGroupMessages: KodyMessage[], allMessages: KodyMessage[]): boolean {
	if (toolGroupMessages.length === 0) {
		return false
	}

	// Step 1: Find the MOST RECENT api_req_started overall (search backwards)
	let mostRecentApiReq: KodyMessage | null = null
 ...
 */
export function getToolsNotInCurrentActivities(toolGroupMessages: KodyMessage[], allMessages: KodyMessage[]): KodyMessage[] {
 ...
 *
 * Note: this operates on a flat `KodyMessage[]` (e.g. `modifiedMessages`) rather than
 ...
 */
export function isApiReqAbsorbable(apiReqTs: number, allMessages: KodyMessage[]): boolean {
	const apiReqIndex = allMessages.findIndex((m) => m.ts === apiReqTs && m.say === "api_req_started")
	if (apiReqIndex === -1) {
		return false
	}
 ...
 */
function isApiReqFollowedOnlyByLowStakesTools(index: number, messages: (KodyMessage | KodyMessage[])[]): boolean {
	let hasLowStakesTool = false
	let hasReasoning = false
 ...
 */
export function groupLowStakesTools(groupedMessages: (KodyMessage | KodyMessage[])[]): (KodyMessage | KodyMessage[])[] {
	const result: (KodyMessage | KodyMessage[])[] = []
	let toolGroup: KodyMessage[] = []
	let pendingReasoning: KodyMessage[] = []
	let pendingApiReq: KodyMessage[] = []
	let hasTools = false
	const pendingTools: KodyMessage[] = []
	...
	const commitToolGroup = () => {
		if (toolGroup.length > 0 && hasTools) {
			const group = toolGroup as KodyMessage[] & { _isToolGroup: boolean }
