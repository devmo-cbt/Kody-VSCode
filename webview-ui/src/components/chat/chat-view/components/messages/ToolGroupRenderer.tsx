import { KodyMessage, KodySayTool } from "@shared/ExtensionMessage"
...
const getCurrentActivities = (allMessages: KodyMessage[]): KodyMessage[] => {
 ...
 */
function buildToolsWithReasoning(messages: KodyMessage[]): ToolWithReasoning[] {
 ...
 */
function parseToolSafe(text: string | undefined): KodySayTool {
	try {
		return JSON.parse(text || "{}") as KodySayTool
	} catch {
		return {} as KodySayTool
	}
}
 ...
 */
function getToolDisplayInfo(tool: KodySayTool) {
 ...
 */
function getToolGroupSummary(messages: KodyMessage[]): string {
	...
	return parts.length === 0 ? "Context" : "Kody" + action + parts.join(", ")
}
