...
import {
	...
	ToggleKodyRuleRequest,
	ToggleSkillRequest,
	ToggleWindsurfRuleRequest,
	ToggleWorkflowRequest,
} from "@shared/proto/kody/file"
...
const KodyRulesToggleModal: React.FC = () => {
	const {
		...
		hooksEnabled,
		setGlobalKodyRulesToggles,
		setLocalKodyRulesToggles,
		...
		if (isVisible) {
			FileServiceClient.refreshRules({} as EmptyRequest)
				.then((response: RefreshedRules) => {
					...
					if (response.globalKodyRulesToggles?.toggles) {
						setGlobalKodyRulesToggles(response.globalKodyRulesToggles.toggles)
					}
					if (response.localKodyRulesToggles?.toggles) {
						setLocalKodyRulesToggles(response.localKodyRulesToggles.toggles)
					}
	...
	const globalRules = Object.entries(globalKodyRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))

	// Format local rules for display with proper typing
	const localRules = Object.entries(localKodyRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))
	...
	const toggleRule = (isGlobal: boolean, rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleKodyRule(
			ToggleKodyRuleRequest.create({
			...
			.catch((error) => {
				console.error("Error toggling Kody rule:", error)
			})
	}
	...
	const toggleWindsurfRule = (rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleWindsurfRule(
			...
			.then((response: KodyRulesToggles) => {
				if (response.toggles) {
	...
	return (
		<div className="inline-flex min-w-0 max-w-full items-center" ref={modalRef}>
			<div className="inline-flex w-full items-center" ref={buttonRef}>
				<Tooltip>
					{!isVisible && <TooltipContent>Manage Kody Rules & Workflows</TooltipContent>}
					<TooltipTrigger>
						<VSCodeButton
							appearance="icon"
							aria-label={isVisible ? "Hide Kody Rules & Workflows" : "Show Kody Rules & Workflows"}
				...
				<PopupModalContainer $arrowPosition={arrowPosition} $menuPosition={menuPosition}>
					...
					<div className="flex-shrink-0 px-3 pt-2">
						...
						<div className="text-xs text-description mb-4">
							{currentView === "rules" ? (
								<p>
									Rules allow you to provide Kody with system-level guidance. Think of them as a persistent way
									to include context and preferences for your projects or globally for every conversation.{" "}
									<VSCodeLink
										className="text-xs"
										href="https://docs.kody.bot/features/kody-rules"
								...
								<p>
									Workflows allow you to define a series of steps to guide Kody through a repetitive set of
									...
									<VSCodeLink
										...
										href="https://docs.kody.bot/features/slash-commands/workflows">
										Docs
									</VSCodeLink>
								</p>
							) : currentView === "skills" ? (
								<p>
									Skills are reusable instruction sets that Kody can activate on-demand. When a task matches a
									skill's description, Kody uses the <span className="font-bold">use_skill</span> tool to load
									the full instructions.
								</p>
							) : (
								<p>
									Hooks allow you to execute custom scripts at specific points in Kody's execution lifecycle,
							...
							<>
								<div className="text-xs text-description mb-4">
									<p>
										...
										<VSCodeLink
											className="text-xs"
											href="https://docs.kody.bot/features/hooks"
											style={{ display: "inline", fontSize: "inherit" }}>
											Docs
										</VSCodeLink>
									</p>
								...
								{workspaceHooks.map((workspace, index) => (
									<div
										...
										<div className="text-sm font-normal mb-2">
											{workspace.workspaceName}/.kodyrules/hooks/
										</div>
										<div className="flex flex-col gap-0">
											{workspace.hooks
												.sort((a, b) => a.name.localeCompare(b.name))
...
export default KodyRulesToggleModal
