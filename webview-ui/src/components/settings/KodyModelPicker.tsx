...
import { EmptyRequest, StringRequest } from "@shared/proto/kody/common"
import { type KodyRecommendedModel, KodyRecommendedModelsResponse } from "@shared/proto/kody/models"
...
export interface KodyModelPickerProps {
	isPopup?: boolean
	currentMode: Mode
...
function toFeaturedModelCardEntry(
	model: Pick<KodyRecommendedModel, "id" | "description" | "tags">,
...
const KodyModelPicker: React.FC<KodyModelPickerProps> = ({ isPopup, currentMode, showProviderRouting, initialTab }) => {
	const { handleModeFieldsChange, handleFieldChange } = useApiConfigurationHandlers()
	const { apiConfiguration, favoritedModelIds, kodyModels, refreshKodyModels } = useExtensionState()
	const modeFields = getModeSpecificFields(apiConfiguration, currentMode)
	const [searchTerm, setSearchTerm] = useState(modeFields.kodyModelId || openRouterDefaultModelId)
	const [isDropdownVisible, setIsDropdownVisible] = useState(false)
	const [selectedIndex, setSelectedIndex] = useState(-1)
	const [kodyRecommendedModels, setKodyRecommendedModels] = useState<FeaturedModelCardEntry[]>([])
	const [kodyFreeModels, setKodyFreeModels] = useState<FeaturedModelCardEntry[]>([])
	const freeKodyModelIds = useMemo(() => {
		const freeModelIds =
			kodyFreeModels.length > 0 ? kodyFreeModels.map((model) => model.id) : FREE_MODELS_FALLBACK.map((model) => model.id)
		return [...new Set(freeModelIds)]
	}, [kodyFreeModels])
	const freeKodyModelIdSet = useMemo(
		() => new Set(freeKodyModelIds.map((modelId) => normalizeModelId(modelId))),
		[freeKodyModelIds],
	)
	const [activeTab, setActiveTab] = useState<"recommended" | "free">(initialTab ?? "recommended")
	const recommendedModels = useMemo(
		() => (kodyRecommendedModels.length > 0 ? kodyRecommendedModels : RECOMMENDED_MODELS_FALLBACK),
		[kodyRecommendedModels],
	)
	const freeModels = useMemo(() => (kodyFreeModels.length > 0 ? kodyFreeModels : FREE_MODELS_FALLBACK), [kodyFreeModels])
	const hasSuccessfulKodyRecommendedModelsFetchRef = useRef(false)
	const isFetchingKodyRecommendedModelsRef = useRef(false)
	const kodyRecommendedModelsRetryTimeoutRef = useRef<number | null>(null)

	const refreshKodyRecommendedModels = useCallback(async (): Promise<boolean> => {
		try {
			const response = await ModelsServiceClient.makeUnaryRequest(
				"refreshKodyRecommendedModelsRpc",
				EmptyRequest.create({}),
				EmptyRequest.toJSON,
				KodyRecommendedModelsResponse.fromJSON,
			)
			...
			const free = (response.free ?? [])
				.map((model) => toFeaturedModelCardEntry(model, "FREE"))
				.filter((model): model is FeaturedModelCardEntry => model !== null)
			setKodyRecommendedModels(recommended)
			setKodyFreeModels(free)
			return true
		} catch (error) {
			console.error("Failed to refresh Kody recommended models:", error)
			return false
		}
	}, [])

	const clearKodyRecommendedModelsRetryTimeout = useCallback(() => {
		if (kodyRecommendedModelsRetryTimeoutRef.current !== null) {
			window.clearTimeout(kodyRecommendedModelsRetryTimeoutRef.current)
			kodyRecommendedModelsRetryTimeoutRef.current = null
		}
	}, [])

	const fetchKodyRecommendedModels = useCallback(async () => {
		if (hasSuccessfulKodyRecommendedModelsFetchRef.current || isFetchingKodyRecommendedModelsRef.current) {
			return
		}
		isFetchingKodyRecommendedModelsRef.current = true
		const succeeded = await refreshKodyRecommendedModels()
		isFetchingKodyRecommendedModelsRef.current = false

		if (succeeded) {
			hasSuccessfulKodyRecommendedModelsFetchRef.current = true
			clearKodyRecommendedModelsRetryTimeout()
			return
		}

		if (kodyRecommendedModelsRetryTimeoutRef.current === null) {
			kodyRecommendedModelsRetryTimeoutRef.current = window.setTimeout(() => {
				kodyRecommendedModelsRetryTimeoutRef.current = null
				void fetchKodyRecommendedModels()
			}, CLINE_RECOMMENDED_MODELS_RETRY_DELAY_MS)
		}
	}, [clearKodyRecommendedModelsRetryTimeout, refreshKodyRecommendedModels])
	...
	useEffect(() => {
		...
		const currentModelId = modeFields.kodyModelId || openRouterDefaultModelId
		setActiveTab(freeKodyModelIdSet.has(normalizeModelId(currentModelId)) ? "free" : "recommended")
	}, [modeFields.kodyModelId, freeKodyModelIdSet, initialTab])
	...
	const handleModelChange = (newModelId: string) => {
		...
		handleModeFieldsChange(
			{
				kodyModelId: { plan: "planModeKodyModelId", act: "actModeKodyModelId" },
				kodyModelInfo: { plan: "planModeKodyModelInfo", act: "actModeKodyModelInfo" },
			},
			{
				kodyModelId: newModelId,
				kodyModelInfo: kodyModels?.[newModelId],
			},
			currentMode,
		)
	}
	...
	const { selectedModelId, selectedModelInfo } = useMemo(() => {
		...
		if (freeKodyModelIdSet.has(normalizeModelId(selected.selectedModelId))) {
			return {
				...selected,
				selectedModelInfo: {
		...
		return selected
	}, [apiConfiguration, currentMode, freeKodyModelIdSet])
	...
	useEffect(() => {
		void fetchKodyRecommendedModels()
	}, [fetchKodyRecommendedModels])

	// Sync external changes when the modelId changes
	useEffect(() => {
		const currentModelId = modeFields.kodyModelId || openRouterDefaultModelId
		setSearchTerm(currentModelId)
	}, [modeFields.kodyModelId])
	...
	const modelIds = useMemo(() => {
		const unfilteredModelIds = Object.keys(kodyModels ?? {}).sort((a, b) => a.localeCompare(b))
		return filterOpenRouterModelIds(unfilteredModelIds, "kody", freeKodyModelIds)
	}, [kodyModels, freeKodyModelIds])
	...
	const showBudgetSlider = useMemo(() => {
		...
		return (
			Object.entries(kodyModels ?? {})?.some(([id, m]) => id === selectedModelId && m.thinkingConfig) ||
	...
	}, [kodyModels, selectedModelId, selectedModelIdLower, showReasoningEffort])
		...
		<div style={{ width: "100%", paddingBottom: 2 }}>
			...
			{hasInfo ? (
				...
				<p
					...
					The extension automatically fetches the latest Kody model list. If you're unsure which model to choose, Kody
...
export default KodyModelPicker
