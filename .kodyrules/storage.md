...
Global settings, secrets and workspace state are stored in **file-backed JSON stores** under `~/.kody/data/`. This is the shared storage layer used by VSCode, CLI, and JetBrains.

## Key Abstractions

### `StorageContext` (src/shared/storage/storage-context.ts)
The entry point. Created via `createStorageContext()` and passed to `StateManager.initialize()`. Contains three `KodyFileStorage` instances:
- `globalState` → `~/.kody/data/globalState.json`
- `secrets` → `~/.kody/data/secrets.json` (mode 0o600)
- `workspaceState` → `~/.kody/data/workspaces/<hash>/workspaceState.json`

### `KodyFileStorage` (src/shared/storage/KodyFileStorage.ts)
...
Remember that your data may be read by a different client than the one that wrote it. For example, a value written by Kody in JetBrains may be read by Kody CLI.
