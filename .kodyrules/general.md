...
- When adding new feature flags, see this PR as a reference https://github.com/kody/kody/pull/7566
- Additional instructions about making requests: @.kodyrules/network.md

## gRPC/Protobuf Communication
The extension and webview communicate via gRPC-like protocol over VS Code message passing.

**Proto files live in `proto/`** (e.g., `proto/kody/task.proto`, `proto/kody/ui.proto`)
- Each feature domain has its own `.proto` file
- For simple data, use shared types in `proto/kody/common.proto` (`StringRequest`, `Empty`, `Int64Request`)
...
**Adding new enum values** (like a new `KodySay` type) requires updating conversion mappings in `src/shared/proto-conversions/kody-message.ts`
...
- `proto/kody/task.proto` - Added `ExplainChangesRequest` message and `explainChanges` RPC
- `proto/kody/ui.proto` - Added `GENERATE_EXPLANATION = 29` to `KodySay` enum
- `src/shared/ExtensionMessage.ts` - Added `KodySayGenerateExplanation` type
- `src/shared/proto-conversions/kody-message.ts` - Added mapping for new say type
...
1. `proto/kody/models.proto` - Add to the `ApiProvider` enum (e.g., `OPENAI_CODEX = 40;`)
...
1. **Add to `KodyDefaultTool` enum** in `src/shared/tools.ts`
2. **Tool definition** in `src/core/prompts/system-prompt/tools/` (create file like `generate_explanation.ts`)
   - Define variants for each `ModelFamily` (generic, next-gen, xs, etc.)
   - Export variants array (e.g., `export const my_tool_variants = [GENERIC, NATIVE_NEXT_GEN, XS]`)
   - **Fallback behavior**: If a variant isn't defined for a model family, `KodyToolSet.getToolByNameWithFallback()` automatically falls back to GENERIC. So you only need to export `[GENERIC]` unless the tool needs model-specific behavior.
...
8. **If tool has UI feedback**: add `KodySay` enum in proto, update `src/shared/ExtensionMessage.ts`, update `src/shared/proto-conversions/kody-message.ts`, update `webview-ui/src/components/chat/ChatRow.tsx`
...
- Add the field to `UpdateSettingsRequest` in `proto/kody/state.proto` (for webview update requests), then run `npm run protos`
