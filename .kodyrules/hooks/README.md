# Kody Hooks Documentation

## Overview

Kody hooks allow you to execute custom scripts at specific points in the agentic workflow. Hooks can be placed in either:
- **Global hooks directory**: `~/Documents/Kody/Hooks/` (applies to all workspaces)
- **Workspace hooks directory**: `.kodyrules/hooks/` (applies to the workspace the repo is part of)

Hooks run automatically when enabled.

## Enabling Hooks

1. Open Kody settings in VSCode
...
- **Global Location**: `~/Documents/Kody/Hooks/TaskStart`
- **Workspace Location**: `.kodyrules/hooks/TaskStart`
...
- **Global Location**: `~/Documents/Kody/Hooks/TaskResume`
- **Workspace Location**: `.kodyrules/hooks/TaskResume`
...
- **Global Location**: `~/Documents/Kody/Hooks/TaskCancel`
- **Workspace Location**: `.kodyrules/hooks/TaskCancel`
...
- **Global Location**: `~/Documents/Kody/Hooks/TaskComplete`
- **Workspace Location**: `.kodyrules/hooks/TaskComplete`
...
- **Global Location**: `~/Documents/Kody/Hooks/UserPromptSubmit`
- **Workspace Location**: `.kodyrules/hooks/UserPromptSubmit`

### PreToolUse Hook
- **When**: Runs BEFORE a tool is executed
- **Purpose**: Validate parameters, block execution, or add context
- **Global Location**: `~/Documents/Kody/Hooks/PreToolUse`
- **Workspace Location**: `.kodyrules/hooks/PreToolUse`

### PostToolUse Hook
- **When**: Runs AFTER a tool completes
- **Purpose**: Observe results, track patterns, or add context
- **Global Location**: `~/Documents/Kody/Hooks/PostToolUse`
- **Workspace Location**: `.kodyrules/hooks/PostToolUse`
...
- **Global Location**: `~/Documents/Kody/Hooks/PreCompact`
- **Workspace Location**: `.kodyrules/hooks/PreCompact`

## Cross-Platform Hook Format

Kody uses a git-style approach for hooks that works consistently across all platforms:
...
Like git hooks, Kody executes hook files through a shell that interprets the shebang line:
...
```bash
# Create hook file
nano ~/Documents/Kody/Hooks/PreToolUse

# Make executable
chmod +x ~/Documents/Kody/Hooks/PreToolUse
```
...
# Log to file
echo "$input" >> ~/.kody/hook-logs/tool-usage.jsonl
...
Kody supports two levels of hooks:

### Global Hooks
- **Location**: `~/Documents/Kody/Hooks/` (macOS/Linux)
...
### Workspace Hooks
- **Location**: `.kodyrules/hooks/` in each workspace root
...
1. The global hooks directory is automatically created at:
   - macOS/Linux: `~/Documents/Kody/Hooks/`
...
3. Enable hooks in Kody settings
...
```bash
#!/usr/bin/env bash
# ~/Documents/Kody/Hooks/PreToolUse
...
If you have multiple workspace roots, you can place hooks in each root's `.kodyrules/hooks/` directory. All hooks (global and workspace) may execute concurrently. Their results will be combined:
...
- Look for errors in VSCode's Output panel (Kody channel)
