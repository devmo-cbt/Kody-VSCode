...
`
	...
	beforeEach(async () => {
		...
		// Create .kodyrules/hooks subdirectory structure
		tempDir = path.join(baseTempDir, ".kodyrules", "hooks")
		await fs.mkdir(tempDir, { recursive: true })
		testHandler = createTestHandler()
		mockMessages = []

		// Mock StateManager to return baseTempDir as workspace root
		// This allows HookFactory to find hooks in baseTempDir/.kodyrules/hooks/
	...
	describe("Error Handling", () => {
		...
		it("should update message state on hook failure", async function () {
			...
			const mockHandler = {
				...testHandler,
				getKodyMessages: () => messages,
				addToKodyMessages: async (msg: KodyMessage) => {
					messages.push(msg)
				},
				updateKodyMessage: async (index: number, updates: Partial<KodyMessage>) => {
					if (messages[index]) {
