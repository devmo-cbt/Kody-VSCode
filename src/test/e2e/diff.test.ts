...
e2e.describe("Diff Editor", () => {
	E2E_WORKSPACE_TYPES.forEach(({ title, workspaceType }) => {
		e2e.extend({
			...
			await inputbox.fill("[diff.test.ts] Hello, Kody!")
			await expect(inputbox).toHaveValue("[diff.test.ts] Hello, Kody!")
			...
			await expect(sidebar.getByText("Hello, Kody!")).toBeVisible() // History with the previous sent message
			...
			await sidebar.waitForSelector('span:has-text("Kody wants to edit this file:")')

			// Kody Diff Editor should open with the file name and diff
			await expect(page.getByText("test.ts: Original ↔ Kody's")).toBeVisible()
