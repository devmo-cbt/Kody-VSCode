...
e2e("Views - can set up API keys and navigate to Settings from Chat", async ({ sidebar }) => {
	...
	await expect(sidebar.getByRole("button", { name: "Login to Kody" })).toBeVisible()
	...
	// Test Kody provider option
	await providerSelectorInput.click({ delay: 100 })
	// Wait for dropdown to appear and find Kody option
	await expect(sidebar.getByTestId("provider-option-kody")).toBeVisible()
	await sidebar.getByTestId("provider-option-kody").click({ delay: 100 })
	await expect(sidebar.getByRole("button", { name: "Sign Up with Kody" })).toBeVisible()
	...
	await expect(sidebar.getByRole("button", { name: "Login to Kody" })).not.toBeVisible()
	...
	// kody logo container
	const kodyLogo = sidebar.locator(".size-20")
	await expect(kodyLogo).toBeVisible()
