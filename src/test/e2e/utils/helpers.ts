...
import { KodyApiServerMock } from "../fixtures/server"
...
export class E2ETestHelper {
	...
	public async signin(webview: Frame): Promise<void> {
		await webview.getByRole("button", { name: "Login to Kody" }).click({ delay: 100 })

		// Verify start up page is no longer visible
		await expect(webview.getByRole("button", { name: "Login to Kody" })).not.toBeVisible()

		await webview.getByRole("button", { name: "Close" }).click({ delay: 50 })
	}

	public static async openKodySidebar(page: Page): Promise<void> {
		await page.getByRole("tab", { name: /Kody/ }).locator("a").click()
	}
...
/**
 * NOTE: Use the `e2e` test fixture for all E2E tests to test the Kody extension.
 *
 * Extended Playwright test configuration for Kody E2E testing.
 *
 * This test configuration provides a comprehensive setup for end-to-end testing of the Kody VS Code extension,
 ...
 *
 * Fixtures provided:
 * - `server`: Shared KodyApiServerMock instance for API mocking (reused across all tests)
 ...
 * - `page`: Playwright Page object representing the main VS Code window with Kody sidebar opened
 * - `sidebar`: Playwright Frame object representing the Kody extension's sidebar iframe
 *
 * @returns Extended test object with all fixtures available for E2E test scenarios:
 * - **server**: Automatically starts and manages a KodyApiServerMock instance
 ...
 * - **page**: Configures the main VS Code window with notifications disabled and Kody sidebar open
 * - **sidebar**: Provides access to the Kody extension's sidebar frame
 *
 * @example
 * ```typescript
 ...
 * - Installs the Kody extension in development mode
...
export const e2e = test
	.extend<{ server: KodyApiServerMock | null }>({
		server: async ({}, use) => {
			// Start server if it doesn't exist
			if (!KodyApiServerMock.globalSharedServer) {
				await KodyApiServerMock.startGlobalServer()
			}
			await use(KodyApiServerMock.globalSharedServer)
		},
	})
	...
	.extend<{ openVSCode: (workspacePath: string) => Promise<ElectronApplication> }>({
		openVSCode: async ({ userDataDir, channel }, use, testInfo) => {
			...
			await use(async (workspacePath: string) => {
				// Create isolated Kody data directory for this test
				const kodyTestDir = mkdtempSync(path.join(os.tmpdir(), "kody-e2e-"))
				...
				const app = await _electron.launch({
					...
					env: {
						...
						CLINE_DIR: kodyTestDir, // Isolate test data from user's ~/.kody
	...
	.extend<{ app: ElectronApplication; kodyTestDir: string }>({
		app: async ({ openVSCode, userDataDir, extensionsDir, workspaceType, workspaceDir, multiRootWorkspaceDir }, use) => {
			...
			// Track the kodyTestDir created in openVSCode
			let kodyTestDir: string | undefined
	...
	.extend<{ sidebar: Frame }>({
		sidebar: async ({ page, helper, server }, use) => {
			await E2ETestHelper.openKodySidebar(page)
