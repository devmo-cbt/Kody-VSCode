...
describe("State Keys Type Safety", () => {
	...
	describe("Type Exports", () => {
		...
		it("should export usable key types", () => {
			...
			const localKey: LocalStateKey = "localKodyRulesToggles"
		...
		it("should have LocalState keys map to KodyRulesToggles", () => {
			const localState: Partial<LocalState> = {
				localKodyRulesToggles: {},
				localCursorRulesToggles: { "some-rule": true },
			}
			expect(localState.localKodyRulesToggles).to.deep.equal({})
		})
	})
	...
	describe("Edge Cases", () => {
		...
		it("should handle keys with special characters", () => {
			// The kody:kodyAccountId key has a colon
			expect(SecretKeys).to.include("kody:kodyAccountId")
			expect(isSecretKey("kody:kodyAccountId")).to.be.true
		})
