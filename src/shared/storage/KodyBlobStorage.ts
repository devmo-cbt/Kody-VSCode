...
import { KodyStorage } from "./KodyStorage"
...
/**
 * S3/R2 blob storage implementation of KodyStorage.
 * Uses AWS S3 or Cloudflare R2 as the backend storage.
 */
export class KodyBlobStorage extends KodyStorage {
	override name = "KodyBlobStorage"

	private static store: KodyBlobStorage | null = null
	static get instance(): KodyBlobStorage {
		if (!KodyBlobStorage.store) {
			KodyBlobStorage.store = new KodyBlobStorage()
		}
		return KodyBlobStorage.store
	}
	...
	public init(settings?: BlobStoreSettings) {
		...
		try {
			if (!KodyBlobStorage.isConfigured(settings)) {
			...
			if (adapter) {
				...
				Logger.log(`[KodyBlobStorage] Adapter created for ${settings.adapterType}`)
			}
		} catch (error) {
			// Log but don't throw - allow startup to continue
			Logger.error("[KodyBlobStorage] initialization failed:", error)
		}
	}
 ...
 */
export const blobStorage = KodyBlobStorage.instance
