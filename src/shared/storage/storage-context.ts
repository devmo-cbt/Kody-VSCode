...
import { KodyFileStorage } from "./KodyFileStorage"
import { KodyMemento } from "./KodyStorage"
...
export interface StorageContext {
	...
	readonly globalState: KodyMemento
	 ...
	 *
	 * This split exists because CLI needs to intercept the KodyMemento interface to global state,
	 * but state resets need to write through to the backing store.
	 */
	readonly globalStateBackingStore: KodyFileStorage

	/** Secrets — API keys and other sensitive values. File uses restricted permissions (0o600). */
	readonly secrets: KodyFileStorage<string>

	/** Workspace-scoped state — per-project toggles, rules, etc. */
	readonly workspaceState: KodyFileStorage

	/** The resolved path to the data directory (~/.kody/data) */
...
export interface StorageContextOptions {
	/**
	 * Override the Kody home directory. Defaults to CLINE_DIR env var or ~/.kody.
	 */
	kodyDir?: string
 ...
 *
 * File layout:
 *   ~/.kody/data/globalState.json    — global state
 *   ~/.kody/data/secrets.json        — secrets (mode 0o600)
 *   ~/.kody/data/workspaces/<hash>/workspaceState.json — per-workspace state
 *
...
export function createStorageContext(opts: StorageContextOptions = {}): StorageContext {
	const kodyDir = opts.kodyDir || process.env.CLINE_DIR || path.join(os.homedir(), ".kody")
	const dataDir = path.join(kodyDir, SETTINGS_SUBFOLDER)
	...
	const globalState = new KodyFileStorage(path.join(dataDir, "globalState.json"), "GlobalState")

	return {
		globalState,
		globalStateBackingStore: globalState,
		secrets: new KodyFileStorage<string>(path.join(dataDir, "secrets.json"), "Secrets", {
			fileMode: 0o600, // Owner read/write only — protects API keys
		}),
		workspaceState: new KodyFileStorage(path.join(workspaceDir, "workspaceState.json"), "WorkspaceState"),
		dataDir,
