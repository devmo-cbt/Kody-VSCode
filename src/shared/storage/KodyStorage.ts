...
export interface KodyStorageChangeEvent {
	readonly key: string
}

export type StorageEventListener = (event: KodyStorageChangeEvent) => Promise<void>
 ...
 * VSCode's Memento and KodySyncStorage both satisfy this interface.
 */
export interface KodyMemento {
 ...
 */
export abstract class KodyStorage {
	/**
	 * The name of the storage, used for logging purposes.
	 */
	protected name = "KodyStorage"
	/**
 ...
 * Unlike KodyStorage (string key-value, async), this stores any JSON-serializable
 * values and provides synchronous access - required for VSCode Memento compatibility.
 */

export type SyncStorageEventListener = (event: KodyStorageChangeEvent) => void

export abstract class KodySyncStorage<T = any> {
...
/**
 * A simple in-memory implementation of KodyStorage using a Map.
 */
export class KodyInMemoryStorage extends KodyStorage {
	/**
