import { KodyMessage } from "./ExtensionMessage"

/**
 * Hook metadata extracted from hook message text.
 * Mirrors the KodySayHook interface but represents parsed data.
 */
...
function getSay(msg: KodyMessage): string | undefined {
 ...
 */
function isToolOrCommandMessage(msg: KodyMessage): boolean {
 ...
 */
function parseHookMetadata(hookMessage: KodyMessage): HookMetadata | null {
 ...
 */
function dedupeToolOrCommandMessagesByTimestamp(messages: KodyMessage[]): KodyMessage[] {
...
function combineHookWithOutputs(
	hookMessage: KodyMessage,
	startIndex: number,
	messages: KodyMessage[],
): { combined: KodyMessage; nextIndex: number } {
 ...
 */
function combineAllHooks(messages: KodyMessage[]): KodyMessage[] {
	// Pass 1: Build map of combined hooks by timestamp
	const combinedHooksByTs = new Map<number, KodyMessage>()
 ...
 */
function findImmediateNextToolTimestamp(hookIndex: number, messages: KodyMessage[]): number | null {
 ...
 */
function buildPreToolUseMap(processedMessages: KodyMessage[], originalMessages: KodyMessage[]): Map<number, KodyMessage[]> {
	const map = new Map<number, KodyMessage[]>()
 ...
 */
function reorderWithPreToolUseHooks(messages: KodyMessage[], preToolUseMap: Map<number, KodyMessage[]>): KodyMessage[] {
	const result: KodyMessage[] = []
 ...
 *
 * @param messages Array of KodyMessage objects to process
 * @returns New array with hooks combined and PreToolUse hooks reordered
 */
export function combineHookSequences(messages: KodyMessage[]): KodyMessage[] {
