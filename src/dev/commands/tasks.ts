...
export function registerTaskCommands(controller: Controller): vscode.Disposable[] {
	return [
		vscode.commands.registerCommand("kody.dev.expireMcpOAuthTokens", async () => {
			try {
		...
		vscode.commands.registerCommand("kody.dev.createTestTasks", async () => {
			const count = (
 ...
 */
function createRealisticMessageSequence(baseTimestamp: number, taskPrompt: string, taskIndex: number): KodyMessage[] {
	...
	const messages: KodyMessage[] = [
		// Initial task message - uses "say" with "text" which is the format used in Kody.ts
		{
			ts: baseTimestamp,
			type: "say",
			say: "text",
