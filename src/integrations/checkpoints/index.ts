...
import { KodyApiReqInfo, KodyMessage, KodySay } from "@shared/ExtensionMessage"
import { getApiMetrics } from "@shared/getApiMetrics"
import { HistoryItem } from "@shared/HistoryItem"
import { KodyCheckpointRestore } from "@shared/WebviewMessage"
...
export class TaskCheckpointManager implements ICheckpointManager {
	...
	async saveCheckpoint(isAttemptCompletionMessage = false, completionMessageTs?: number): Promise<void> {
		try {
			...
			const kodyMessages = this.services.messageStateHandler.getKodyMessages()
			kodyMessages.forEach((message) => {
			...
			if (!isAttemptCompletionMessage) {
				...
				if (messageTs) {
					const messages = this.services.messageStateHandler.getKodyMessages()
						...
						this.state.checkpointTracker
							...
							.then(async (commitHash) => {
								if (commitHash) {
									...
									await this.services.messageStateHandler.saveKodyMessagesAndUpdateHistory()
								}
							})
							.catch((error) => {
								Logger.error(
				...
				const lastFivekodyMessages = this.services.messageStateHandler.getKodyMessages().slice(-3)
				const lastCompletionResultMessage = findLast(lastFivekodyMessages, (m) => m.say === "completion_result")
	...
	async restoreCheckpoint(
		messageTs: number,
		restoreType: KodyCheckpointRestore,
		offset?: number,
	): Promise<CheckpointRestoreStateUpdate> {
		try {
			const kodyMessages = this.services.messageStateHandler.getKodyMessages()
			const messageIndex = kodyMessages.findIndex((m) => m.ts === messageTs) - (offset || 0)
			// Find the last message before messageIndex that has a lastCheckpointHash
			const lastHashIndex = findLastIndex(kodyMessages.slice(0, messageIndex), (m) => m.lastCheckpointHash !== undefined)
			const message = kodyMessages[messageIndex]
			const lastMessageWithHash = kodyMessages[lastHashIndex]
	...
	async presentMultifileDiff(messageTs: number, seeNewChangesSinceLastTaskCompletion: boolean): Promise<void> {
		...
		try {
			...
			const kodyMessages = this.services.messageStateHandler.getKodyMessages()
			const messageIndex = kodyMessages.findIndex((m) => m.ts === messageTs)
			const message = kodyMessages[messageIndex]
			if (!message) {
				Logger.error(`[TaskCheckpointManager] Message not found for timestamp ${messageTs} in task ${this.task.taskId}`)
				relinquishButton()
			...
			if (seeNewChangesSinceLastTaskCompletion) {
				...
				const lastTaskCompletedMessageCheckpointHash = findLast(
					this.services.messageStateHandler.getKodyMessages().slice(0, messageIndex),
	...
	async doesLatestTaskCompletionHaveNewChanges(): Promise<boolean> {
		try {
			...
			const kodyMessages = this.services.messageStateHandler.getKodyMessages()
			const messageIndex = findLastIndex(kodyMessages, (m) => m.say === "completion_result")
			const message = kodyMessages[messageIndex]
			if (!message) {
	...
	private async handleSuccessfulRestore(
		...
		switch (restoreType) {
			...
			case "taskAndWorkspace":
				...
				const kodyMessages = this.services.messageStateHandler.getKodyMessages()
				const deletedMessages = kodyMessages.slice(messageIndex + 1)
				...
				const newKodyMessages = kodyMessages.slice(0, messageIndex + 1)
				await this.services.messageStateHandler.overwriteKodyMessages(newKodyMessages) // calls saveKodyMessages which saves historyItem
		...
		try {
			checkpointsWarningTimer = setTimeout(async () => {
				if (!checkpointsWarningShown) {
					...
					await this.setcheckpointManagerErrorMessage(
						"Checkpoints are taking longer than expected to initialize. Working in a large repository? Consider re-opening Kody in a project that uses git, or disabling checkpoints.",
					)
				}
			}, 7_000)
			...
			const tracker = await pTimeout(
				CheckpointTracker.create(this.task.taskId, this.config.enableCheckpoints, workspacePath),
				{
					milliseconds: 15_000,
					message:
						"Checkpoints taking too long to initialize. Consider re-opening Kody in a project that uses git, or disabling checkpoints.",
				},
			)
			...
			if (errorMessage.includes("Checkpoints taking too long to initialize")) {
				await this.setcheckpointManagerErrorMessage(
					"Checkpoints initialization timed out. Consider re-opening Kody in a project that uses git, or disabling checkpoints.",
				)
			} else {
				await this.setcheckpointManagerErrorMessage(errorMessage)
			}
