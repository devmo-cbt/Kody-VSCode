...
export async function orchestrateCommandExecution(
	...
	const clearCommandState = async () => {
		...
		const kodyMessages = callbacks.getKodyMessages()
		const lastCommandIndex = findLastIndex(kodyMessages, (m) => m.ask === "command" || m.say === "command")
		if (lastCommandIndex !== -1) {
			await callbacks.updateKodyMessage(lastCommandIndex, {
	...
	const switchToFileBased = async () => {
		...
		// Set up file logging using KodyTempManager for proper cleanup
		largeOutputLogPath = KodyTempManager.createTempFilePath("large-output")
