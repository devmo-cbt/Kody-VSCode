...
import { KodyError } from "./KodyError"
import { IErrorProvider } from "./providers/IErrorProvider"

/**
 * ErrorService handles error logging and tracking for the Kody extension
...
export class ErrorService {
	...
	captureException(error: Error | KodyError, properties?: Record<string, unknown>) {
		return this.provider.captureException(error, properties)
	}

	public logException(error: Error | KodyError, properties?: Record<string, unknown>): void {
	...
	public toKodyError(rawError: unknown, modelId?: string, providerId?: string): KodyError {
		const transformed = KodyError.transform(rawError, modelId, providerId)
