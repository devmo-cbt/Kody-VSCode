...
export class KodyError extends Error {
	...
	/**
	 * Parses a stringified error into a KodyError instance.
	 */
	static parse(errorStr?: string, modelId?: string): KodyError | undefined {
		if (!errorStr || typeof errorStr !== "string") {
			return undefined
		}
		return KodyError.transform(errorStr, modelId)
	}

	/**
	 * Transforms any object into a KodyError instance.
	 * Always returns a KodyError, even if the input is not a valid error object.
	 */
	static transform(error: any, modelId?: string, providerId?: string): KodyError {
		try {
			// If already a KodyError, return it directly to prevent infinite recursion
			if (error instanceof KodyError) {
				return error
			}
			return new KodyError(JSON.parse(error), modelId, providerId)
		} catch {
			return new KodyError(error, modelId, providerId)
		}
	}

	public isErrorType(type: KodyErrorType): boolean {
		return KodyError.getErrorType(this) === type
	}
	 ...
	 */
	static getErrorType(err: KodyError): KodyErrorType | undefined {
		...
		if (message) {
			...
			if (RATE_LIMIT_PATTERNS.some((pattern) => pattern.test(lowerMessage))) {
				return KodyErrorType.RateLimit
			}
		}
...
export class AuthNetworkError extends Error {
	constructor(
		...
		super(message)
		this.name = KodyErrorType.Network
	}
}
