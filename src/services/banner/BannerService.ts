import type { Banner, BannerAction, BannerRules, BannersResponse } from "@shared/KodyBanner"
import { BannerActionType, type BannerCardData } from "@shared/kody/banner"
...
export class BannerService {
	...
	public async sendBannerEvent(bannerId: string, eventType: "dismiss"): Promise<void> {
		try {
			const url = new URL("/banners/v2/messages", KodyEnv.config().apiBaseUrl).toString()
	...
	private buildFetchUrl(): string {
		const url = new URL("/banners/v2/messages", KodyEnv.config().apiBaseUrl)
