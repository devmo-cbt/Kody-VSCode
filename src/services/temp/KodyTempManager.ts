/**
 * KodyTempManager - Manages temporary files for Kody with automatic cleanup.
 *
 * Simple approach:
 * - Uses a "kody" subdirectory inside the system temp dir (falls back to system temp if creation fails)
...
/**
 * Singleton manager for Kody's temporary files.
 */
class KodyTempManagerImpl {
	private readonly tempDir: string
	private cleanupIntervalId: NodeJS.Timeout | null = null

	constructor() {
		// Uses system temp directory with a dedicated "kody" subdirectory when possible:
		// macOS: /var/folders/xx/.../T/kody
		// Windows: C:\Users\{user}\AppData\Local\Temp\kody
		// Linux: /tmp/kody
		const baseTempDir = os.tmpdir()
		const kodyTempDir = path.join(baseTempDir, "kody")

		try {
			fs.mkdirSync(kodyTempDir, { recursive: true })
			this.tempDir = kodyTempDir
		} catch {
	...
	/**
	 * Clean up old Kody temp files based on age and total size constraints.
	 * Called on extension activation.
	 *
	 * Strategy:
	 * 1. Scan the Kody temp directory
	...
	async cleanup(): Promise<{ deletedCount: number; freedBytes: number }> {
		...
		try {
			...
			if (deletedCount > 0) {
				Logger.info(`Kody temp cleanup: deleted ${deletedCount} files, freed ${Math.round(freedBytes / 1024 / 1024)}MB`)
			}
		} catch (error) {
			Logger.error("Error during Kody temp cleanup", error)
		}
...
export const KodyTempManager = new KodyTempManagerImpl()
