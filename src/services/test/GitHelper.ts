...
export async function validateWorkspacePath(workspacePath: string): Promise<void> {
	...
	try {
		const testFile = path.join(workspacePath, ".kody_write_test")
...
export async function initializeGitRepository(workspacePath: string): Promise<boolean> {
	...
	try {
		...
		await execa("git", ["config", "user.name", "Kody Evaluation"], { cwd: workspacePath })
		await execa("git", ["config", "user.email", "kody@example.com"], { cwd: workspacePath })
