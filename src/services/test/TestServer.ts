import { getSavedApiConversationHistory, getSavedKodyMessages } from "@core/storage/disk"
...
export async function createTestServer(controller: Controller): Promise<http.Server> {
	// Try to show the Kody sidebar
	Logger.log("[createTestServer] Opening Kody in sidebar...")
		...
		req.on("end", async () => {
			try {
				...
				if (!visibleWebview || !visibleWebview.controller) {
					res.writeHead(500)
					res.end(JSON.stringify({ error: "No active Kody instance found" }))
					return
				}
				...
				try {
					...
					if (apiKey) {
						...
						const updatedConfig = {
							...apiConfiguration,
							apiProvider: "kody" as ApiProvider,
							kodyAccountId: apiKey,
						}

						// Store the API key securely
						visibleWebview.controller.stateManager.setSecret("kodyAccountId", apiKey)

						visibleWebview.controller.stateManager.setApiConfiguration(updatedConfig)

						// Update cache service to use kody provider
					...
					try {
						...
						try {
							if (typeof taskId === "string") {
								messages = await getSavedKodyMessages(taskId)
							}
						} catch (error) {
							Logger.log(`Error getting saved Kody messages: ${error}`)
						}
