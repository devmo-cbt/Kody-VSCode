import { String } from "@shared/proto/kody/common"
...
export class AuthServiceMock extends AuthService {
	...
	override async getAuthToken(): Promise<string | null> {
		...
		return this._kodyAuthInfo.idToken
	}

	override async createAuthRequest(): Promise<String> {
		// Use URL object for more graceful query construction
		const authUrl = new URL(KodyEnv.config().apiBaseUrl)
		const authUrlString = authUrl.toString()
		// Call the parent implementation
		if (this._authenticated && this._kodyAuthInfo) {
			Logger.log("Already authenticated with mock server")
			return String.create({ value: authUrlString })
		}

		try {
			// Use token exchange endpoint like KodyAuthProvider
			const tokenExchangeUri = new URL(CLINE_API_ENDPOINT.TOKEN_EXCHANGE, KodyEnv.config().apiBaseUrl)
