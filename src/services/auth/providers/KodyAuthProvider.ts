...
import { EnvironmentConfig, KodyEnv } from "@/config"
import { Controller } from "@/core/controller"
import { HostProvider } from "@/hosts/host-provider"
import { buildBasicKodyHeaders } from "@/services/EnvUtils"
import { AuthInvalidTokenError, AuthNetworkError } from "@/services/error/KodyError"
import { telemetryService } from "@/services/telemetry"
import { CLINE_API_ENDPOINT } from "@/shared/kody/api"
import { fetch, getAxiosSettings } from "@/shared/net"
import { Logger } from "@/shared/services/Logger"
import { type KodyAccountUserInfo, type KodyAuthInfo } from "../AuthService"
import { parseJwtPayload } from "../oca/utils/utils"

interface KodyAuthApiUser {
	subject: string | null
	email: string
	name: string
...
interface KodyAuthResponseData {
	/**
...
export interface KodyAuthApiTokenExchangeResponse {
	success: boolean
	data: KodyAuthResponseData
}

export interface KodyAuthApiTokenRefreshResponse {
	success: boolean
	data: KodyAuthResponseData
}

export class KodyAuthProvider {
	readonly name = "kody"
	...
	private clearSession(controller: Controller, reason: string, storedAuthData?: KodyAuthInfo) {
		Logger.error(reason)
		...
		controller.stateManager.setSecret("kody:kodyAccountId", undefined)
		this.refreshRetryCount = 0
		this.lastRefreshAttempt = 0
		return null
	}

	private logFailedRefreshAttempt(response: Response, storedAuthData?: KodyAuthInfo) {
		const startedAt = storedAuthData?.startedAt
		const timeSinceStarted = Date.now() - (startedAt || 0)
	...
	/**
	 * Retrieves Kody auth info using the stored access token.
	 * @param controller - The controller instance to access stored secrets.
	 * @returns {Promise<KodyAuthInfo | null>} A promise that resolves with the auth info or null.
	 */
	async retrieveKodyAuthInfo(controller: Controller): Promise<KodyAuthInfo | null> {
		try {
			// Get the stored auth data from secure storage
			const storedAuthDataString = controller.stateManager.getSecretKey("kody:kodyAccountId")
			...
			let storedAuthData: KodyAuthInfo
			try {
			...
			if (await this.shouldRefreshIdToken(storedAuthData.refreshToken, storedAuthData.expiresAt)) {
				...
				try {
					...
					if (newAuthInfoString !== storedAuthDataString) {
						controller.stateManager.setSecret("kodyAccountId", undefined) // cleanup old key
						controller.stateManager.setSecret("kody:kodyAccountId", newAuthInfoString)
					}
					...
					// when the user actually tries to use Kody, not at startup
					return storedAuthData
				}
			}
	 ...
	 * @returns {Promise<KodyAuthInfo>} The new access token and user info.
	 */
	async refreshToken(refreshToken: string, storedData: KodyAuthInfo): Promise<KodyAuthInfo> {
		try {
			...
			const data: KodyAuthApiTokenExchangeResponse = await response.json()
	...
	async signIn(controller: Controller, authorizationCode: string, provider: string): Promise<KodyAuthInfo | null> {
		try {
			...
			const responseType: KodyAuthApiTokenExchangeResponse = responseJSON
			...
			controller.stateManager.setSecret("kody:kodyAccountId", JSON.stringify(kodyAuthInfo))
	...
	private async fetchRemoteUserInfo(tokenData: KodyAuthApiTokenExchangeResponse["data"]): Promise<KodyAccountUserInfo> {
		try {
			const userResponse = await axios.get(`${KodyEnv.config().apiBaseUrl}/api/v1/users/me`, {
				headers: {
			...
			return {
				id: tokenData.userInfo.kodyUserId || "",
