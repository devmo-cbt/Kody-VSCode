...
export function transformRemoteConfigToStateShape(remoteConfig: RemoteConfig): Partial<RemoteConfigFields> {
	...
	const kodySettings = remoteConfig.providerSettings?.Kody
	if (kodySettings) {
		transformed.planModeApiProvider = "kody"
		transformed.actModeApiProvider = "kody"
		providers.push("kody")
	}
...
export function clearRemoteConfig() {
	try {
		...
		// the remote config kody rules toggle state is stored in global state
