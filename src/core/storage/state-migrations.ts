...
export async function migrateMcpMarketplaceEnableSetting(mcpMarketplaceEnabledRaw: boolean | undefined): Promise<boolean> {
	const config = vscode.workspace.getConfiguration("kody")
...
export async function migrateCustomInstructionsToGlobalRules(context: vscode.ExtensionContext) {
	try {
		...
		if (customInstructions?.trim()) {
			Logger.log("Migrating custom instructions to global Kody rules...")

			// Create global .kodyrules directory if it doesn't exist
			...
			Logger.log("Successfully migrated custom instructions to global Kody rules")
		}
	} catch (error) {
...
export async function migrateWelcomeViewCompleted(context: vscode.ExtensionContext) {
	try {
		...
		if (welcomeViewCompleted === undefined) {
			...
			const kodyAccountId = await context.secrets.get("kodyAccountId")
...
export async function cleanupOldApiKey(context: vscode.ExtensionContext) {
	try {
		...
		await context.secrets.delete("kodyApiKey")
	} catch (error) {
		Logger.error("Failed to cleanup old kodyApiKey", error)
	}
}
