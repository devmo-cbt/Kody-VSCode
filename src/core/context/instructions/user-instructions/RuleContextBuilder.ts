...
type MessageStateHandlerLike = {
	getKodyMessages(): KodyMessageLike[]
}
...
/**
 * Builds the evaluation context used for conditional Kody Rules (e.g. YAML frontmatter `paths:`).
 *
 * Kept in the user-instructions domain so Task remains orchestration-focused.
 *
 * Path context is gathered from multiple sources in kodyMessages:
...
export class RuleContextBuilder {
	...
	private static async getRulePathContext(deps: RuleContextBuilderDeps): Promise<string[]> {
		...
		const kodyMessages = deps.messageStateHandler.getKodyMessages()
		...
		const lastUserMsg = [...kodyMessages]
			.reverse()
		...
		// (3) Files edited by Kody during this task (completed operations):
