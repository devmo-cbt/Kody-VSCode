...
describe("Skills Utility Functions", () => {
	...
	const GLOBAL_SKILLS_DIR = path.join("/home", "user", ".kody", "skills")
	...
	beforeEach(() => {
		...
		sandbox.stub(disk, "getSkillsDirectoriesForScan").returns([
			{ path: path.join(TEST_CWD, ".kodyrules", "skills"), source: "project" },
			{ path: path.join(TEST_CWD, ".kody", "skills"), source: "project" },
...
Instructions here`)
		...
		it("should discover skills from project .kodyrules/skills directory", async () => {
			const projectSkillsDir = path.join(TEST_CWD, ".kodyrules", "skills")
...
Use analogies and ASCII diagrams when explaining code.`)
		...
		it("should discover skills from project .kody/skills directory", async () => {
			const kodySkillsDir = path.join(TEST_CWD, ".kody", "skills")
			const skillDir = path.join(kodySkillsDir, "debugging")
			const skillMdPath = path.join(skillDir, "SKILL.md")

			fileExistsStub.withArgs(kodySkillsDir).resolves(true)
			fileExistsStub.withArgs(skillMdPath).resolves(true)
			isDirectoryStub.withArgs(kodySkillsDir).resolves(true)
			readdirStub.withArgs(kodySkillsDir).resolves(["debugging"])
			statStub.withArgs(skillDir).resolves({ isDirectory: () => true })
			readFileStub.withArgs(skillMdPath, "utf-8").resolves(`---
name: debugging
