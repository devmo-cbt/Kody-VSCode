...
import { KodyIgnoreController } from "./KodyIgnoreController"
import "should"

describe("KodyIgnoreController", () => {
	let tempDir: string
	let controller: KodyIgnoreController
	...
	beforeEach(async () => {
		...
		// Create default .kodyignore file
		await fs.writeFile(
			path.join(tempDir, ".kodyignore"),
			[".env", "*.secret", "private/", "# This is a comment", "", "temp.*", "file-with-space-at-end.* ", "**/.git/**"].join(
				"\n",
			),
		)

		controller = new KodyIgnoreController(tempDir)
	...
	describe("Default Patterns", () => {
		...
		it("should block access to .kodyignore file", async () => {
			const result = controller.validateAccess(".kodyignore")
	...
	describe("Custom Patterns", () => {
		...
		it("should handle comments in .kodyignore", async () => {
			// Create a new .kodyignore with comments
			await fs.writeFile(path.join(tempDir, ".kodyignore"), ["# Comment line", "*.secret", "private/", "temp.*"].join("\n"))
	...
	describe("Error Handling", () => {
		...
		it("should handle missing .kodyignore gracefully", async () => {
			// Create a new controller in a directory without .kodyignore
			const emptyDir = path.join(os.tmpdir(), `llm-test-empty-${Date.now()}`)
			await fs.mkdir(emptyDir)

			try {
				const controller = new KodyIgnoreController(emptyDir)
				await controller.initialize()
		...
		it("should handle empty .kodyignore", async () => {
			await fs.writeFile(path.join(tempDir, ".kodyignore"), "")
	...
	describe("Include Directive", () => {
		it("should load patterns from an included file", async () => {
			...
			// Create a .kodyignore file that includes .gitignore and adds an extra pattern "secret.txt"
			await fs.writeFile(path.join(tempDir, ".kodyignore"), ["!include .gitignore", "secret.txt"].join("\n"))

			// Initialize the controller to load the updated .kodyignore
			controller = new KodyIgnoreController(tempDir)
			await controller.initialize()
			...
			// "secret.txt" should be ignored as specified directly in .kodyignore
		...
		it("should handle non-existent included file gracefully", async () => {
			// Create a .kodyignore file that includes a non-existent file
			await fs.writeFile(path.join(tempDir, ".kodyignore"), ["!include missing-file.txt"].join("\n"))
		...
		it("should handle non-existent included file gracefully alongside a valid pattern", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".kodyignore"), ["!include non-existent.txt", "*.tmp"].join("\n"))
