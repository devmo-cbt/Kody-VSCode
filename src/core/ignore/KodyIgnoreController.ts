 ...
 * Designed to be instantiated once in Kody.ts and passed to file manipulation services.
 * Uses the 'ignore' library to support standard .gitignore syntax in .kodyignore files.
 */
export class KodyIgnoreController {
	private cwd: string
	private ignoreInstance: Ignore
	private fileWatcher?: FSWatcher
	kodyIgnoreContent: string | undefined

	constructor(cwd: string) {
		this.cwd = cwd
		this.ignoreInstance = ignore()
		this.kodyIgnoreContent = undefined
	}
	...
	async initialize(): Promise<void> {
		// Set up file watcher for .kodyignore
		this.setupFileWatcher()
		await this.loadKodyIgnore()
	}

	/**
	 * Set up the file watcher for .kodyignore changes
	 */
	private setupFileWatcher(): void {
		const ignorePath = path.join(this.cwd, ".kodyignore")
		...
		this.fileWatcher.on("error", (error) => {
			Logger.error("Error watching .kodyignore file:", error)
		})
	}

	/**
	 * Load custom patterns from .kodyignore if it exists.
	 * Supports "!include <filename>" to load additional ignore patterns from other files.
	 */
	private async loadKodyIgnore(): Promise<void> {
		try {
			...
			if (await fileExistsAtPath(ignorePath)) {
				...
				this.kodyIgnoreContent = content
				await this.processIgnoreContent(content)
				this.ignoreInstance.add(".kodyignore")
			} else {
				this.kodyIgnoreContent = undefined
			}
		} catch (error) {
			// Should never happen: reading file failed even though it exists
			Logger.error("Unexpected error loading .kodyignore:", error)
		}
	}
	...
	private async processIgnoreContent(content: string): Promise<void> {
		...
		const combinedContent = await this.processKodyIgnoreIncludes(content)
	 ...
	 */
	private async processKodyIgnoreIncludes(content: string): Promise<string> {
		let combinedContent = ""
	...
	private async readIncludedFile(includeLine: string): Promise<string | null> {
		...
		if (!(await fileExistsAtPath(resolvedIncludePath))) {
			Logger.debug(`[KodyIgnore] Included file not found: ${resolvedIncludePath}`)
			return null
		}
	...
	validateAccess(filePath: string): boolean {
		// Always allow access if .kodyignore does not exist
		if (!this.kodyIgnoreContent) {
			return true
		}
		try {
	...
	validateCommand(command: string): string | undefined {
		// Always allow if no .kodyignore exists
		if (!this.kodyIgnoreContent) {
			return undefined
		}
