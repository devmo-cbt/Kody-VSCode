...
import { type KodyToolSpec, toolSpecFunctionDeclarations, toolSpecFunctionDefinition, toolSpecInputSchema } from "../spec"
import { PromptVariant, SystemPromptContext } from "../types"

export class KodyToolSet {
	// A list of tools mapped by model group
	private static variants: Map<ModelFamily, Set<KodyToolSet>> = new Map()

	private constructor(
		public readonly id: string,
		public readonly config: KodyToolSpec,
	) {
		this._register()
	}

	public static register(config: KodyToolSpec): KodyToolSet {
		return new KodyToolSet(config.id, config)
	}

	private _register(): void {
		const existingTools = KodyToolSet.variants.get(this.config.variant) || new Set()
		if (!Array.from(existingTools).some((t) => t.config.id === this.config.id)) {
			existingTools.add(this)
			KodyToolSet.variants.set(this.config.variant, existingTools)
		}
	}

	public static getTools(variant: ModelFamily): KodyToolSet[] {
		const toolsSet = KodyToolSet.variants.get(variant) || new Set()
		const defaultSet = KodyToolSet.variants.get(ModelFamily.GENERIC) || new Set()

		return toolsSet ? Array.from(toolsSet) : Array.from(defaultSet)
	}

	public static getRegisteredModelIds(): string[] {
		return Array.from(KodyToolSet.variants.keys())
	}

	public static getToolByName(toolName: string, variant: ModelFamily): KodyToolSet | undefined {
		const tools = KodyToolSet.getTools(variant)
	...
	public static getToolByNameWithFallback(toolName: string, variant: ModelFamily): KodyToolSet | undefined {
		// Try exact variant first
		const exact = KodyToolSet.getToolByName(toolName, variant)
		if (exact) {
			return exact
		}

		// Fallback to GENERIC
		const generic = KodyToolSet.getToolByName(toolName, ModelFamily.GENERIC)
		if (generic) {
			return generic
		}

		// Final fallback: search across all registered variants
		for (const [, tools] of KodyToolSet.variants) {
	...
	public static getToolsForVariantWithFallback(variant: ModelFamily, requestedIds: string[]): KodyToolSet[] {
		const resolved: KodyToolSet[] = []
		for (const id of requestedIds) {
			const tool = KodyToolSet.getToolByNameWithFallback(id, variant)
			if (tool) {
	...
	public static getEnabledTools(variant: PromptVariant, context: SystemPromptContext): KodyToolSet[] {
		const resolved: KodyToolSet[] = []
		const requestedIds = variant.tools ? [...variant.tools] : []
		for (const id of requestedIds) {
			const tool = KodyToolSet.getToolByNameWithFallback(id, variant.family)
			if (tool) {
				// Avoid duplicates by id
				if (!resolved.some((t) => t.config.id === tool.config.id)) {
					resolved.push(tool)
	...
	private static getDynamicSubagentToolSpecs(variant: PromptVariant, context: SystemPromptContext): KodyToolSpec[] {
		if (context.subagentsEnabled !== true || context.isSubagentRun) {
			return []
		}

		const requestedIds = variant.tools ? [...variant.tools] : []
		const shouldIncludeSubagentTools = requestedIds.length === 0 || requestedIds.includes(KodyDefaultTool.USE_SUBAGENTS)
		...
		return agentConfigs.map(({ toolName, config }) => ({
			...
			id: KodyDefaultTool.USE_SUBAGENTS,
			name: toolName,
	...
	public static getEnabledToolSpecs(variant: PromptVariant, context: SystemPromptContext): KodyToolSpec[] {
		const registeredTools = KodyToolSet.getEnabledTools(variant, context).map((tool) => tool.config)
		const dynamicSubagentTools = KodyToolSet.getDynamicSubagentToolSpecs(variant, context)

		const includesDynamicSubagents = dynamicSubagentTools.length > 0
		const filteredRegistered = includesDynamicSubagents
			? registeredTools.filter((tool) => tool.id !== KodyDefaultTool.USE_SUBAGENTS)
			: registeredTools
	...
	public static getNativeTools(variant: PromptVariant, context: SystemPromptContext) {
		...
		// Base set
		const toolConfigs = KodyToolSet.getEnabledToolSpecs(variant, context)

		// MCP tools
		const mcpServers = context.mcpHub?.getServers()?.filter((s) => s.disabled !== true) || []
		const mcpTools = mcpServers?.flatMap((server) => mcpToolToKodyToolSpec(variant.family, server))

		const enabledTools = [...toolConfigs, ...mcpTools].filter(
			(tool) => typeof tool.description === "string" && tool.description.trim().length > 0,
		)
		const converter = KodyToolSet.getNativeConverter(context.providerInfo.providerId, context.providerInfo.model.id)
...
/**
 * Convert an MCP server's tools to KodyToolSpec format
 */
export function mcpToolToKodyToolSpec(family: ModelFamily, server: McpServer): KodyToolSpec[] {
