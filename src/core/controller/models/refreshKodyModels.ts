...
/**
 * The raw model information returned by the Kody API to list models
 */
interface KodyRawModelInfo {
	id: string
	name: string
	...
	tiers?: any[] | null
	supported_parameters?: KodySupportedParams[] | null
}
...
async function fetchRawKodyModels(): Promise<KodyRawModelInfo[]> {
	const apiBaseUrl = KodyEnv.config().apiBaseUrl
	const response = await axios.get(`${apiBaseUrl}/api/v1/ai/kody/models`, getAxiosSettings())

	if (!Array.isArray(response.data?.data)) {
		throw new Error("Invalid response data when fetching Kody models")
	}

	Logger.log("Kody models source: Kody API")
	return response.data.data as KodyRawModelInfo[]
}

/**
 * Core function: Refreshes the Kody models and returns application types
 * @param controller The controller instance
 * @returns Record of model ID to ModelInfo (application types)
 */
export async function refreshKodyModels(controller: Controller): Promise<Record<string, ModelInfo>> {
	const shouldUseKodyEndpointSource = featureFlagsService.getBooleanFlagEnabled(FeatureFlag.EXTENSION_CLINE_MODELS_ENDPOINT)
	if (!shouldUseKodyEndpointSource) {
		return refreshOpenRouterModels(controller)
	}

	// Check in-memory cache first
	const cache = StateManager.get().getModelsCache("kody")
	if (cache) {
		return cache
	}
	...
	pendingRefresh = (async () => {
		try {
			return await fetchAndCacheKodyModels()
		} finally {
...
async function fetchAndCacheKodyModels(): Promise<Record<string, ModelInfo>> {
	const kodyModelsFilePath = path.join(await ensureCacheDirectoryExists(), GlobalFileNames.kodyModels)

	let models: Record<string, ModelInfo> = {}
	try {
		const rawModels = await fetchRawKodyModels()
		...
		if (Object.keys(models).length === 0) {
			throw new Error("No Kody models returned from API")
		}
		// Save models and cache them in memory
		await fs.writeFile(kodyModelsFilePath, JSON.stringify(models))
		Logger.log("Kody models fetched and saved")
	} catch (error) {
		Logger.error("Error fetching Kody models:", error)

		// If we failed to fetch models, try to read cached models from disk
		try {
			const fileExists = await fileExistsAtPath(kodyModelsFilePath)
			if (fileExists) {
				const fileContents = await fs.readFile(kodyModelsFilePath, "utf8")
				models = JSON.parse(fileContents)
				Logger.log("Loaded Kody models from cache")
			}
		} catch (cacheError) {
			Logger.error("Error reading Kody models from cache:", cacheError)
		}
	}

	// Avoid poisoning in-memory cache with an empty model map after transient failures.
	if (Object.keys(models).length > 0) {
		StateManager.get().setModelsCache("kody", models)
	}
...
/**
 * Read cached Kody models from disk
 * @returns The cached models or undefined if not found
 */
export async function readKodyModelsFromCache(): Promise<Record<string, ModelInfo> | undefined> {
	try {
		const kodyModelsFilePath = path.join(await ensureCacheDirectoryExists(), GlobalFileNames.kodyModels)
		const fileExists = await fileExistsAtPath(kodyModelsFilePath)
		if (fileExists) {
		...
		Logger.error("Error reading Kody models from cache:", error)
	}
	return undefined
}
