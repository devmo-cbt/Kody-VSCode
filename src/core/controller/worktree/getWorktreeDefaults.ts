...
import { WorktreeDefaults } from "@shared/proto/kody/worktree"
...
export async function getWorktreeDefaults(_controller: Controller, _request: EmptyRequest): Promise<WorktreeDefaults> {
	...
	const suggestedBranch = `worktree/kody-${suffix}`

	// Generate suggested path in Documents/Kody/Worktrees/<project-name>-<suffix>
	...
	const suggestedPath = path.join(documentsPath, "Kody", "Worktrees", `${projectName}-${suffix}`)
