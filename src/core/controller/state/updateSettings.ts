...
import { PlanActMode, McpDisplayMode as ProtoMcpDisplayMode, UpdateSettingsRequest } from "@shared/proto/kody/state"
...
export async function updateSettings(controller: Controller, request: UpdateSettingsRequest): Promise<Empty> {
	try {
		if (request.kodyEnv !== undefined) {
			KodyEnv.setEnvironment(request.kodyEnv)
		...
		// Update kody web tools setting
		if (request.kodyWebToolsEnabled !== undefined) {
			if (controller.task) {
				telemetryService.captureKodyWebToolsToggle(controller.task.ulid, request.kodyWebToolsEnabled)
			}
			controller.stateManager.setGlobalState("kodyWebToolsEnabled", request.kodyWebToolsEnabled)
		}
		...
		if (request.focusChainSettings !== undefined) {
			{
				...
				const focusChainSettings = {
					enabled: isEnabled,
					remindKodyInterval: request.focusChainSettings.remindKodyInterval,
				}
