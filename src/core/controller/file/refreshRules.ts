...
import { refreshKodyRulesToggles } from "@core/context/instructions/user-instructions/kody-rules"
import { refreshWorkflowToggles } from "@core/context/instructions/user-instructions/workflows"
import { EmptyRequest } from "@shared/proto/kody/common"
import { RefreshedRules } from "@shared/proto/kody/file"
import { Logger } from "@/shared/services/Logger"
import { getCwd, getDesktopDir } from "@/utils/path"
import type { Controller } from "../index"

/**
 * Refreshes all rule toggles (Kody, External, and Workflows)
...
export async function refreshRules(controller: Controller, _request: EmptyRequest): Promise<RefreshedRules> {
	try {
		...
		const { globalToggles, localToggles } = await refreshKodyRulesToggles(controller, cwd)
