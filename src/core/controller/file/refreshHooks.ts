import { HookInfo, HooksToggles, WorkspaceHooks } from "@shared/proto/kody/file"
...
export async function refreshHooks(
	...
	const globalHooksDir = globalHooksDirOverride || path.join(os.homedir(), "Documents", "Kody", "Hooks")
	...
	for (const workspacePath of workspacePaths.paths) {
		const workspaceHooksDir = path.join(workspacePath, ".kodyrules", "hooks")
