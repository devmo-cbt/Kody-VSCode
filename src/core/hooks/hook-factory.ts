...
import { version as kodyVersion } from "../../../package.json"
 ...
 * provide--the other common parameters like kodyVersion and userId are handled by the
 * hook system.
 */
...
export abstract class HookRunner<Name extends HookName> {
	 ...
	 * - kodyVersion: Current Kody extension version
	 ...
	 * - userId: Kody user ID, machine ID, or generated UUID
	 *
	...
	protected async completeParams(params: NamedHookInput<Name>): Promise<HookInput> {
		...
		return {
			...
			workspaceRoots,
			userId: getDistinctId(), // Always available: Kody User ID, machine ID, or generated UUID
			...params,
			model,
		}
	}
 ...
 *
 * Used in multi-root workspaces where both global hooks (from ~/Documents/Kody/Hooks/)
 * and workspace-specific hooks (from each workspace's .kodyrules/hooks/) exist for the
 * same hook type.
 *
 * Behavior:
...
function isExpectedHookError(error: unknown): boolean {
	...
	// Note: This is expected because users may have hooks in .kodyrules that they don't want to execute
...
export class HookFactory {
	 ...
	 * Global hooks are located in paths containing "Kody/Hooks" or "kody/hooks".
	 */
	 ...
	 *
	 * - Global hooks (from ~/Documents/Kody/Hooks/): run from the primary workspace root
	 * - Workspace hooks (from workspaceRoot/.kodyrules/hooks/): run from that specific workspace root
	 *
	...
	private determineHookCwd(
		...
		// Workspace hooks are at: workspaceRoot/.kodyrules/hooks/
	 ...
	 * Global hooks are located in ~/Documents/Kody/Hooks/
	 * Workspace hooks are located in workspace .kodyrules/hooks/ directories
	 *
	 ...
	 * Includes both global hooks (from ~/Documents/Kody/Hooks/) and workspace hooks
	 * (from .kodyrules/hooks/ in each workspace root).
	 */
	...
	/**
	 * Finds the path to a hook in a .kodyrules hooks directory.
	 *
	 * @param hookName the name of the hook to search for, for example 'PreToolUse'
	 * @param hooksDir the .kodyrules directory path to search
