...
/**
 * All valid hook types that can be created and executed by Kody.
...
export async function resolveHooksDirectory(
	...
	if (isGlobal) {
		return globalHooksDirOverride || path.join(os.homedir(), "Documents", "Kody", "Hooks")
	}
	...
	if (workspaceName) {
		...
		return path.join(targetWorkspace, ".kodyrules", "hooks")
	}

	// Single workspace: use getCwd
	const cwd = await getCwd(getDesktopDir())
	return path.join(cwd, ".kodyrules", "hooks")
}
