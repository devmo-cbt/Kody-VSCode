...
export async function parseSlashCommands(
...
): Promise<{ processedText: string; needsKodyrulesFileCheck: boolean }> {
	...
	for (const { regex } of tagPatterns) {
		...
		if (tagMatch) {
			...
			if (SUPPORTED_DEFAULT_COMMANDS.includes(commandName)) {
				...
				return { processedText: processedText, needsKodyrulesFileCheck: commandName === "newrule" }
			}
				...
				if (parts.length >= 3) {
					...
					try {
						...
						if (promptResponse) {
							...
							return { processedText, needsKodyrulesFileCheck: false }
						}
	...
	return { processedText: text, needsKodyrulesFileCheck: false }
}
