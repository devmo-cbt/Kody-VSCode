import { getSavedKodyMessages, getTaskMetadata, readTaskHistoryFromState, writeTaskHistoryToState } from "@core/storage/disk"
...
async function reconstructTaskHistoryItem(taskId: string): Promise<HistoryItem | null> {
	try {
		...
		const kodyMessages = await getSavedKodyMessages(taskId)
		if (kodyMessages.length === 0) {
		...
		const taskInfo = extractTaskInformation(kodyMessages, metadata)
...
function extractTaskInformation(kodyMessages: KodyMessage[], metadata: any): TaskInfo {
	// Find the first user message (task description)
	const firstUserMessage = kodyMessages.find((msg) => msg.type === "say" && msg.say === "text" && msg.text)

	// Extract timestamp from first message or use task ID as fallback
	const timestamp = kodyMessages.length > 0 ? kodyMessages[0].ts : Date.now()
	...
	const apiReqMessages = kodyMessages.filter(
	...
	const messageSize = JSON.stringify(kodyMessages).length
