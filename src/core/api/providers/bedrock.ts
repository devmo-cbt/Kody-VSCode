...
import type { KodyTool } from "@/shared/tools"
...
export class AwsBedrockHandler implements ApiHandler {
	...
	async *createMessage(systemPrompt: string, messages: KodyStorageMessage[], tools?: KodyTool[]): ApiStream {
	...
	private async getAwsCredentials(): Promise<{
		...
		const providerOptions: ProviderChainOptions = {
			clientConfig: {
				...
				userAgentAppId: `kody#${ExtensionRegistryInfo.version}`,
			},
		}
		const useProfile =
	 ...
	 */
	private formatDeepseekR1Prompt(systemPrompt: string, messages: KodyStorageMessage[]): string {
	 ...
	 */
	private estimateInputTokens(systemPrompt: string, messages: KodyStorageMessage[]): number {
	...
	/**
	 * Converts Kody's tool definitions (Anthropic format with `input_schema`) to the
	 ...
	 */
	private mapKodyToolsToBedrockToolConfig(tools?: KodyTool[]): ToolConfiguration | undefined {
		if (!tools || tools.length === 0) {
			return undefined
		}

		const isAnthropicTool = (tool: KodyTool): tool is AnthropicTool => "input_schema" in tool
	...
	private async *createAnthropicMessage(
		...
		const toolConfig = this.mapKodyToolsToBedrockToolConfig(tools)
	 ...
	 */
	private formatMessagesForConverseAPI(messages: KodyStorageMessage[]): Message[] {
