...
export class DifyHandler implements ApiHandler {
	...
	async *createMessage(systemPrompt: string, messages: KodyStorageMessage[]): ApiStream {
		...
		const requestBody = {
			...
			user: "kody-user", // A unique user identifier
			files: [],
		}
	...
	private convertMessagesToQuery(systemPrompt: string, messages: KodyStorageMessage[]): string {
	 ...
	 * @param user User identifier (defaults to "kody-user")
	 * @returns Promise with file upload response
	 */
	async uploadFile(file: Buffer, filename: string, user = "kody-user"): Promise<DifyFileResponse> {
	 ...
	 */
	async stopGeneration(taskId: string, user = "kody-user"): Promise<void> {
	 ...
	 */
	async deleteConversation(conversationId: string, user = "kody-user"): Promise<void> {
