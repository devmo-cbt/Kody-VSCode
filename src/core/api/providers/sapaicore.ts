...
namespace Bedrock {
	 ...
	 */
	export function formatMessagesForConverseAPI(messages: KodyStorageMessage[]): BedrockMessage[] {
...
export class SapAiCoreHandler implements ApiHandler {
	...
	private async *createMessageWithOrchestration(systemPrompt: string, messages: KodyStorageMessage[]): ApiStream {
		try {
			...
			// sent directly to the LLM with the Kody system prompt without validation errors.
	...
	private async *createMessageWithDeployments(systemPrompt: string, messages: KodyStorageMessage[]): ApiStream {
		const token = await this.getToken()
		const externalHeaders = buildExternalBasicHeaders()
		const headers = {
			...externalHeaders,
	...
	private convertMessageParamToSAPMessages(messages: KodyStorageMessage[]): ChatMessage[] {
