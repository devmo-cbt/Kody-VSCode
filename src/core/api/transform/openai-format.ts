 ...
 *
 * @param toolId - The original tool ID from Kody/Anthropic format
...
/**
 * Converts an array of KodyStorageMessage objects to OpenAI's Completions API format.
 *
 * Handles conversion of Kody-specific content types (tool uses, tool results, images, reasoning details)
 * into OpenAI's expected message structure, including tool_calls and tool_call_id fields.
 *
 * @param anthropicMessages - Array of KodyStorageMessage objects to be converted
...
export function convertToOpenAiMessages(
	anthropicMessages: Omit<KodyStorageMessage, "modelInfo">[],
	...
	for (const anthropicMessage of anthropicMessages) {
         ...
         */
			if (anthropicMessage.role === "user") {
				...
				const toolResultImages: KodyImageContentBlock[] = []
				...
				const tool_calls: OpenAI.Chat.ChatCompletionMessageToolCall[] = toolMessages.map((toolMessage) => {
					...
					if (toolDetails) {
						if (Array.isArray(toolDetails)) {
							...
							// See: https://github.com/kody/kody/issues/8214
...
const UNIQUE_ERROR_TOOL_NAME = "_kody_error_unknown_function_"
