...
import { convertKodyStorageToAnthropicMessage, KodyStorageMessage } from "@/shared/messages/content"

/**
 * Converts Kody storage messages to Anthropic API format with optional cache control.
 * Adds ephemeral cache control to the last two user messages to prevent them from being
 * stored in Anthropic's cache.
 *
 * @param kodyMessages - Array of Kody storage messages to convert
...
export function sanitizeAnthropicMessages(
	kodyMessages: Array<KodyStorageMessage | Anthropic.MessageParam>,
	...
	const userMsgIndices = kodyMessages.reduce((acc, msg, index) => {
		if (msg.role === "user") {
			acc.push(index)
		}
		return acc
	...
	return kodyMessages.map((msg, index) => {
		const anthropicMsg = convertKodyStorageToAnthropicMessage(msg)
