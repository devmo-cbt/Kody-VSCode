...
import { KodyAssistantThinkingBlock, KodyStorageMessage } from "@/shared/messages/content"
...
export function addReasoningContent(
	...
	originalMessages: KodyStorageMessage[],
		...
		if (msg.role === "assistant") {
			if (Array.isArray(msg.content)) {
				const thinking = msg.content
					.filter((p): p is KodyAssistantThinkingBlock => p.type === "thinking")
					.map((p) => p.thinking)
					.join("\n")
				if (thinking) {
