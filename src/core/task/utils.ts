...
import { KodyApiReqCancelReason, KodyApiReqInfo } from "@/shared/ExtensionMessage"
...
export const updateApiReqMsg = async (params: UpdateApiReqMsgParams) => {
	const kodyMessages = params.messageStateHandler.getKodyMessages()
	const currentApiReqInfo: KodyApiReqInfo = JSON.parse(kodyMessages[params.lastApiReqIndex].text || "{}")
	delete currentApiReqInfo.retryStatus // Clear retry status when request is finalized

	await params.messageStateHandler.updateKodyMessage(params.lastApiReqIndex, {
		text: JSON.stringify({
