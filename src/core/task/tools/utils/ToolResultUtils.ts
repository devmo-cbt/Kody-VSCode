...
export class ToolResultUtils {
	...
	static pushToolResult(
		...
		if (typeof content === "string") {
			...
			// Get tool_use_id from map using call_id, or use "kody" as fallback for backward compatibility
			const toolUseId = toolUseIdMap?.get(block.call_id || "") || "kody"
			...
			// If using backward-compatible "kody" ID and content is an array, spread it directly
			// instead of wrapping it (which would cause JSON.stringify in createToolResultBlock)
			if ((toolUseId === "kody" || !toolUseId) && Array.isArray(content)) {
	...
	private static createToolResultBlock(content: ToolResponse, id?: string, call_id?: string) {
		// If id is "kody", we treat it as a plain text result for backward compatibility
	 ...
	 */
	static async askApprovalAndPushFeedback(type: KodyAsk, completeMessage: string, config: TaskConfig) {
