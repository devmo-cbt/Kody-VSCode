...
import type { KodyIgnoreController } from "@core/ignore/KodyIgnoreController"
...
export class ToolValidator {
	constructor(private readonly kodyIgnoreController: KodyIgnoreController) {}
	...
	/**
	 * Verifies access is allowed to a given path via .kodyignore rules.
	 * Callers should pass a repo-relative (workspace-relative) path.
	 */
	checkKodyIgnorePath(relPath: string): ValidationResult {
		const accessAllowed = this.kodyIgnoreController.validateAccess(relPath)
		if (!accessAllowed) {
			return {
				ok: false,
				error: `Access to path '${relPath}' is blocked by .kodyignore settings.`,
			}
		}
		return { ok: true }
	}
