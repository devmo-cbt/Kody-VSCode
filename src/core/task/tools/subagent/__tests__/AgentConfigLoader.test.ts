...
import { getToolUseNames, KodyDefaultTool } from "@/shared/tools"
...
You are a code reviewer.`
		...
		assert.deepEqual(parsed.tools, [KodyDefaultTool.FILE_READ, KodyDefaultTool.LIST_FILES, KodyDefaultTool.SEARCH])
		assert.equal(parsed.systemPrompt, "You are a code reviewer.")
	})

	it("supports raw Kody tool ids in tools", () => {
		const content = `---
name: cli-agent
...
Prompt body`
		...
		const parsed = parseAgentConfigFromYaml(content)
		assert.deepEqual(parsed.tools, [KodyDefaultTool.FILE_READ, KodyDefaultTool.LIST_FILES])
	})
...
Prompt body`
	...
	it("loads all yaml/yml files from homeDir/.kody/data/agents", async () => {
		const tempHome = await createTempHomeDir()
		tempDirs.push(tempHome)
...
Reviewer prompt`,
		...
		assert.deepEqual(localAgent?.tools, [KodyDefaultTool.FILE_READ])
		assert.equal(localAgent?.systemPrompt, "Prompt body")
		assert.equal(reviewer?.name, "reviewer")
		assert.deepEqual(reviewer?.tools, [KodyDefaultTool.LIST_FILES])
