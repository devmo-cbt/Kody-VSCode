...
import { KodyToolSet } from "@core/prompts/system-prompt/registry/KodyToolSet"
...
describe("SubagentBuilder", () => {
	...
	it("uses cached config by subagent name and applies act-mode provider model override", () => {
			...
			getCachedConfig: (subagentName?: string) =>
				subagentName === "cached-agent"
					? {
							...
							tools: [KodyDefaultTool.LIST_FILES],
							modelId: "gpt-5",
		...
		assert.deepEqual(builder.getAllowedTools(), [KodyDefaultTool.LIST_FILES, KodyDefaultTool.ATTEMPT])
			...
			getCachedConfig: (subagentName?: string) =>
				subagentName === "openrouter-agent"
					? {
							...
							tools: [KodyDefaultTool.FILE_READ],
	...
	it("builds native tools by filtering allowed ids and context requirements then converting", () => {
		...
		const getToolsStub = sinon.stub(KodyToolSet, "getToolsForVariantWithFallback").returns([
			{
				config: {
					id: KodyDefaultTool.LIST_FILES,
					contextRequirements: () => true,
				},
			},
			{
				config: {
					id: KodyDefaultTool.SEARCH,
					contextRequirements: () => true,
				},
			},
			{
				config: {
					id: KodyDefaultTool.ATTEMPT,
					contextRequirements: () => false,
				},
			},
		] as never)
		const converter = sinon.stub().callsFake((tool: { id: string }) => ({ converted: tool.id }))
		const getConverterStub = sinon.stub(KodyToolSet, "getNativeConverter").returns(converter as never)
		...
		assert.deepEqual(result, [{ converted: KodyDefaultTool.LIST_FILES }])
	})
})
