...
export const SUBAGENT_DEFAULT_ALLOWED_TOOLS: KodyDefaultTool[] = [
...
export class SubagentBuilder {
	...
	getAllowedTools(): KodyDefaultTool[] {
	...
	buildNativeTools(context: SystemPromptContext) {
		...
		const toolSets = KodyToolSet.getToolsForVariantWithFallback(family, this.allowedTools)
	...
	private resolveAllowedTools(configuredTools?: KodyDefaultTool[]): KodyDefaultTool[] {
		const sourceTools = configuredTools && configuredTools.length > 0 ? configuredTools : SUBAGENT_DEFAULT_ALLOWED_TOOLS
		return Array.from(new Set([...sourceTools, KodyDefaultTool.ATTEMPT]))
	}
