...
import { KodyDefaultTool, setDynamicToolUseNames } from "@shared/tools"
...
/** Default Directory for agent configurations: ~/Documents/Kody/Agents */
...
const AgentBaseConfigSchema = z.object({
	...
	tools: z.array(z.nativeEnum(KodyDefaultTool)).default([]),
...
function normalizeToolName(toolName: string): KodyDefaultTool {
	const trimmed = toolName.trim()
	if (!trimmed) {
		throw new Error("Tool name cannot be empty.")
	}

	const asDefaultTool = trimmed as KodyDefaultTool
	if (Object.values(KodyDefaultTool).includes(asDefaultTool)) {
		return asDefaultTool
	}

	throw new Error(
		`Unknown tool '${trimmed}'. Expected a KodyDefaultTool value (for example: read_file, list_files, search_files).`,
	)
}

function parseTools(tools: string | string[] | undefined): KodyDefaultTool[] {
	if (!tools) {
		return []
	}
...
export function getAgentsConfigPath(homeDir = os.homedir()): string {
	return path.join(homeDir, "Documents", "Kody", AGENTS_CONFIG_DIRECTORY_NAME)
}
