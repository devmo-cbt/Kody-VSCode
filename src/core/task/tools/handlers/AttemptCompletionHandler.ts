...
export class AttemptCompletionHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = KodyDefaultTool.ATTEMPT
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		const addNewChangesFlagToLastCompletionResultMessage = async () => {
			...
			const kodyMessages = config.messageState.getKodyMessages()

			const lastCompletionResultMessageIndex = findLastIndex(kodyMessages, (m: any) => m.say === "completion_result")
			const lastCompletionResultMessage =
				lastCompletionResultMessageIndex !== -1 ? kodyMessages[lastCompletionResultMessageIndex] : undefined
			if (
				...
				await config.messageState.updateKodyMessage(lastCompletionResultMessageIndex, {
		...
		if (partialCompletionIndex !== -1) {
			const updatedMessages = [
				...kodyMessages.slice(0, partialCompletionIndex),
				...kodyMessages.slice(partialCompletionIndex + 1),
			]
			config.messageState.setKodyMessages(updatedMessages)
			await config.messageState.saveKodyMessagesAndUpdateHistory()
		}

		let commandResult: any
		const lastMessage = config.messageState.getKodyMessages().at(-1)
		...
		if (command) {
			...
			const autoApproveResult = config.autoApprover?.shouldAutoApproveTool(KodyDefaultTool.BASH)
		...
		if (config.messageState.getKodyMessages().at(-1)?.ask === "command_output") {
