...
import { KodyAsk, KodySay } from "@shared/ExtensionMessage"
import { KodyContent } from "@shared/messages/content"
import { KodyDefaultTool, toolUseNames } from "@shared/tools"
...
export function canonicalizeAttemptCompletionParams(block: ToolUse): boolean {
	if (block.name === KodyDefaultTool.ATTEMPT && !block.params?.result && typeof block.params?.response === "string") {
...
export class ToolExecutor {
	...
	private shouldAutoApproveTool(toolName: KodyDefaultTool): boolean | [boolean, boolean] {
		return this.autoApprover.shouldAutoApproveTool(toolName)
	}

	private async shouldAutoApproveToolWithPath(
		blockname: KodyDefaultTool,
	...
	constructor(
		...
		private sayAndCreateMissingParamError: (toolName: KodyDefaultTool, paramName: string, relPath?: string) => Promise<any>,
		private removeLastPartialMessageIfExistsWithType: (type: "ask" | "say", askOrSay: KodyAsk | KodySay) => Promise<void>,
	...
	private asToolConfig(): TaskConfig {
		const config: TaskConfig = {
			...
			services: {
				...
				kodyIgnoreController: this.kodyIgnoreController,
	...
	private registerToolHandlers(): void {
		const validator = new ToolValidator(this.kodyIgnoreController)
	 ...
	 */
	private static readonly PLAN_MODE_RESTRICTED_TOOLS: KodyDefaultTool[] = [
		KodyDefaultTool.FILE_NEW,
		KodyDefaultTool.FILE_EDIT,
		KodyDefaultTool.NEW_RULE,
		KodyDefaultTool.APPLY_PATCH,
	]
	 ...
	 */
	private isPlanModeToolRestricted(toolName: KodyDefaultTool): boolean {
