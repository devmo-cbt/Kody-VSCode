...
describe("multifile-diff", () => {
	...
	describe("showChangedFilesDiff", () => {
		...
		const mockMessages: KodyMessage[] = [
			{
				ts: mockMessageTs,
				type: "say",
		...
		beforeEach(() => {
			messageStateHandlerStub.getKodyMessages.returns(mockMessages)
		})
		...
		it("should successfully show diff for changes since last task completion", async () => {
			...
			const messagesWithCompletion: KodyMessage[] = [
				{
					ts: 1234567000,
					type: "say",
			...
			messageStateHandlerStub.getKodyMessages.returns(messagesWithCompletion)
		...
		it("should handle message not found error", async () => {
			// Arrange
			messageStateHandlerStub.getKodyMessages.returns([])
		...
		it("should handle missing checkpoint hash", async () => {
			// Arrange
			const messagesWithoutHash: KodyMessage[] = [
				{
					ts: mockMessageTs,
					type: "say",
					say: "text",
			...
			messageStateHandlerStub.getKodyMessages.returns(messagesWithoutHash)
		...
		it("should use first checkpoint when no last completion found", async () => {
			// Arrange
			const messagesWithFirstCheckpoint: KodyMessage[] = [
				{
					ts: 1234567000,
					type: "say",
			...
			messageStateHandlerStub.getKodyMessages.returns(messagesWithFirstCheckpoint)
