...
import { Environment, KodyConfigurationError, KodyEndpoint, KodyEnv } from "../config"

describe("KodyEndpoint configuration", () => {
	...
	beforeEach(async () => {
		...
		// Create .kody directory
		await fs.mkdir(path.join(tempDir, ".kody"), { recursive: true })
		...
		;(KodyEndpoint as any)._instance = null
		;(KodyEndpoint as any)._initialized = false
		;(KodyEndpoint as any)._extensionFsPath = undefined
	})
	...
	describe("valid config parsing", () => {
		it("should parse valid endpoints.json with all required fields", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".kody", "endpoints.json"), JSON.stringify(validConfig), "utf8")

			await KodyEndpoint.initialize(tempDir)

			const config = KodyEndpoint.config
		...
		it("should work without endpoints.json (standard mode)", async () => {
			...
			config.appBaseUrl.should.equal("https://app.kody.bot")
			config.apiBaseUrl.should.equal("https://api.kody.bot")
		})
		...
		it("should accept URLs with paths", async () => {
			const validConfig = {
				appBaseUrl: "https://proxy.enterprise.com/kody/app",
				apiBaseUrl: "https://proxy.enterprise.com/kody/api",
				mcpBaseUrl: "https://proxy.enterprise.com/kody/mcp",
			}
			...
			const config = KodyEndpoint.config
			config.appBaseUrl.should.equal("https://proxy.enterprise.com/kody/app")
		})
	})

	describe("invalid JSON handling", () => {
		it("should throw KodyConfigurationError for invalid JSON syntax", async () => {
			await fs.writeFile(path.join(tempDir, ".kody", "endpoints.json"), "{ invalid json }", "utf8")

			try {
				await KodyEndpoint.initialize(tempDir)
				throw new Error("Should have thrown")
			} catch (error: any) {
				error.should.be.instanceof(KodyConfigurationError)
				error.message.should.containEql("Invalid JSON")
			}
		})

		it("should throw KodyConfigurationError for truncated JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".kody", "endpoints.json"), '{"appBaseUrl": "https://test.com"', "utf8")
		...
		it("should throw KodyConfigurationError for empty file", async () => {
			await fs.writeFile(path.join(tempDir, ".kody", "endpoints.json"), "", "utf8")
		...
		it("should throw KodyConfigurationError for non-object JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".kody", "endpoints.json"), '"just a string"', "utf8")
		...
		it("should throw KodyConfigurationError for array JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".kody", "endpoints.json"), "[]", "utf8")
		...
		it("should throw KodyConfigurationError for null JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".kody", "endpoints.json"), "null", "utf8")
	...
	describe("missing required fields", () => {
		it("should throw KodyConfigurationError when appBaseUrl is missing", async () => {
			const config = {
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
			}

			await fs.writeFile(path.join(tempDir, ".kody", "endpoints.json"), JSON.stringify(config), "utf8")
		...
		it("should throw KodyConfigurationError when apiBaseUrl is missing", async () => {
			const config = {
				appBaseUrl: "https://app.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
			}
		...
		it("should throw KodyConfigurationError when mcpBaseUrl is missing", async () => {
			const config = {
				appBaseUrl: "https://app.enterprise.com",
				apiBaseUrl: "https://api.enterprise.com",
			}
		...
		it("should throw KodyConfigurationError when all fields are missing", async () => {
			await fs.writeFile(path.join(tempDir, ".kody", "endpoints.json"), "{}", "utf8")
		...
		it("should throw KodyConfigurationError when field is null", async () => {
			const config = {
				appBaseUrl: null,
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
		...
		it("should throw KodyConfigurationError when field is empty string", async () => {
			const config = {
				appBaseUrl: "",
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
		...
		it("should throw KodyConfigurationError when field is whitespace only", async () => {
			const config = {
				appBaseUrl: "   ",
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
		...
		it("should throw KodyConfigurationError when field is non-string", async () => {
			const config = {
				appBaseUrl: 12345,
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
	...
	describe("invalid URL detection", () => {
		it("should throw KodyConfigurationError for invalid URL format", async () => {
			const config = {
		...
		it("should throw KodyConfigurationError for URL without protocol", async () => {
			const config = {
		...
		it("should throw KodyConfigurationError for malformed URL", async () => {
			const config = {
				appBaseUrl: "https://",
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
	...
	describe("environment switching blocked in self-hosted mode", () => {
		it("should throw error when trying to change environment in self-hosted mode", async () => {
			...
			KodyEndpoint.config.environment.should.equal(Environment.selfHosted)

			// Try to change environment - should throw
			try {
				KodyEnv.setEnvironment("staging")
				throw new Error("Should have thrown")
			} catch (error: any) {
		...
		it("should allow environment switching in standard mode", async () => {
			...
			KodyEndpoint.config.environment.should.not.equal(Environment.selfHosted)

			// Should be able to change environment
			KodyEnv.setEnvironment("staging")
			KodyEnv.getEnvironment().environment.should.equal("staging")

			KodyEnv.setEnvironment("local")
			KodyEnv.getEnvironment().environment.should.equal("local")

			KodyEnv.setEnvironment("production")
			KodyEnv.getEnvironment().environment.should.equal("production")
		})
	})
	...
	describe("self-hosted mode behavior", () => {
		it("should report selfHosted environment in self-hosted mode", async () => {
			...
			const envConfig = KodyEndpoint.config
		...
		it("should use custom endpoints from file", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".kody", "endpoints.json"), JSON.stringify(customConfig), "utf8")
	...
	describe("initialization behavior", () => {
		it("should only initialize once", async () => {
			await KodyEndpoint.initialize(tempDir)
			KodyEndpoint.isInitialized().should.be.true()
	...
	describe("isSelfHosted() method", () => {
		it("should return true when not initialized (safety fallback)", async () => {
			...
			KodyEndpoint.isInitialized().should.be.false()
			KodyEndpoint.isSelfHosted().should.be.true()
		})
		...
		it("should return false when in normal mode (no endpoints.json)", async () => {
			...
			KodyEndpoint.isSelfHosted().should.be.false()
		})
	})
	...
	describe("bundled endpoints.json behavior", () => {
		...
		it("should use bundled endpoints.json when available", async () => {
			...
			await KodyEndpoint.initialize(bundledDir)
		...
		it("should prefer bundled endpoints.json over user file", async () => {
			...
			// Set up both configs
			await fs.writeFile(path.join(bundledDir, "endpoints.json"), JSON.stringify(bundledConfig), "utf8")
			await fs.writeFile(path.join(tempDir, ".kody", "endpoints.json"), JSON.stringify(userConfig), "utf8")
		...
		it("should throw KodyConfigurationError for invalid bundled file", async () => {
			const invalidConfig = {
		...
		it("should throw KodyConfigurationError for invalid JSON in bundled file", async () => {
