...
describe("Filesystem Utilities", () => {
	const tmpDir = path.join(os.tmpdir(), "kody-test-" + Math.random().toString(36).slice(2))
	...
	it("should exclude .kodyrules/workflows directory specifically", async () => {
		// Create a test directory structure
		const kodyrulesDirTest = path.join(tmpDir, "kodyrules-test")
		const kodyrulesDirPath = path.join(kodyrulesDirTest, ".kodyrules")

		// Create .kodyrules directory and root files
		await fs.mkdir(kodyrulesDirPath, { recursive: true })
		await fs.writeFile(path.join(kodyrulesDirPath, "config.json"), "{}")
		await fs.writeFile(path.join(kodyrulesDirPath, "settings.js"), "// settings")

		// Create .kodyrules/other directory and files
		const otherDirPath = path.join(kodyrulesDirPath, "other")
		...
		// Create .kodyrules/workflows directory and files
		const workflowsDirPath = path.join(kodyrulesDirPath, "workflows")
		...
		const allFiles = await readDirectory(kodyrulesDirPath)
		...
		const filteredFiles = await readDirectory(kodyrulesDirPath, [[".kodyrules", "workflows"]])

		// Verify workflows files are excluded but others remain
		filteredFiles.length.should.equal(4) // 2 in root + 2 in other

		const expectedFiles = [
			path.resolve(kodyrulesDirPath, "config.json"),
			path.resolve(kodyrulesDirPath, "settings.js"),
		...
		const multiExcludeFiles = await readDirectory(kodyrulesDirPath, [
			[".kodyrules", "workflows"],
			[".kodyrules", "other"],
		])
		...
		const rootOnlyFiles = [path.resolve(kodyrulesDirPath, "config.json"), path.resolve(kodyrulesDirPath, "settings.js")]

		multiExcludeFiles.sort().should.deepEqual(rootOnlyFiles.sort())
	})

	it("should exclude .kodyrules/hooks directory specifically", async () => {
		// Create a test directory structure
		const kodyrulesDirTest = path.join(tmpDir, "kodyrules-hooks-test")
		const kodyrulesDirPath = path.join(kodyrulesDirTest, ".kodyrules")
		...
		// Create .kodyrules/hooks directory and files
		const hooksDirPath = path.join(kodyrulesDirPath, "hooks")
		...
		const filteredFiles = await readDirectory(kodyrulesDirPath, [[".kodyrules", "hooks"]])
